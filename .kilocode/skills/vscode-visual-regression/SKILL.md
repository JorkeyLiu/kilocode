...
3. **Baselines** are Linux-only Chromium PNGs stored under `packages/kilo-docs/public/img/screenshot-tests/kilo-vscode/` (tracked via Git LFS)
...
- **Snapshot path** is derived automatically: `packages/kilo-docs/public/img/screenshot-tests/kilo-vscode/{title-slug}/{variant-slug}-chromium-linux.png`. The `-chromium-linux` suffix is part of the snapshot file name — Linux Chromium is the single canonical pixel-baseline environment, and the test runner passes the suffixed name to `toHaveScreenshot()` because Playwright's `snapshotPathTemplate` (`{arg}{ext}`) does not append it automatically. Example: `chat/chat-view-idle-chromium-linux.png`.
...
Baselines are generated on **Linux CI only** (font rendering differs on macOS). PR runs are read-only compare: `visual-regression.yml` runs `bun run test:visual` (no `--update-snapshots`) and fails with uploaded Playwright results when screenshots differ. Baselines are regenerated by the **acceptance** mode of the same workflow — a maintainer triggers it manually (`workflow_dispatch`) with `ref` set to the branch and `scope: kilo-vscode`. The acceptance test job checks out the branch with no persisted credentials, runs `test:visual:update` on Linux, removes stale baselines, re-runs the compare to prove stability, and uploads the regenerated baseline directory as an artifact together with the exact tested `HEAD` revision. A separate isolated `commit-baselines` job then downloads the artifact, checks out the same branch with `lfs: false` and no persisted credentials, aborts unless the recorded tested revision matches its fresh checkout HEAD (a branch that advanced mid-run is rejected), overlays and verifies the baseline tree, and only in its final step configures a narrowly scoped `GITHUB_TOKEN` (`contents: write`) and pushes a single commit. Target-branch local actions, package scripts and `.lfsconfig` never run in the credentialed job.

**You do NOT need to generate baseline PNGs locally.** Just write the story, push, and if the PR reports changed baselines, ask a maintainer to run the acceptance workflow for the PR branch.
...
Snapshots live under `packages/kilo-docs/public/img/screenshot-tests/kilo-vscode/`:

```
packages/kilo-docs/public/img/screenshot-tests/kilo-vscode/
  {title-slug}/
    {variant-slug}-chromium-linux.png
```

The title-slug is derived from the meta `title` (lowercased, slashes become hyphens). The variant-slug is derived from the story export name (kebab-cased). The `-chromium-linux` suffix is part of the snapshot file name passed by the test runner — Playwright's `snapshotPathTemplate` (`{arg}{ext}`) does not append it automatically.
...
The `visual-regression.yml` workflow has two modes:

| Mode | Trigger | What it does |
|---|---|---|
| Compare (PR) | `pull_request` when these paths change | Runs `bun run test:visual` read-only on Linux. Fails and uploads `visual-regression-vscode-results` when screenshots differ. Never writes, commits, or pushes. |
| Acceptance | `workflow_dispatch` (`ref` = branch, `scope` = `all` or `kilo-vscode`) | The visual test job runs `bun run test:visual:update` and stale cleanup with no write token, re-runs compare to prove stability, and uploads the regenerated baseline directory as an artifact together with the exact tested `HEAD` revision. The isolated `commit-baselines` job downloads it, checks out the branch with `lfs: false` and no persisted credentials, aborts unless the recorded tested revision matches its fresh checkout HEAD, overlays and verifies the tree, then pushes a single commit with a narrowly scoped `GITHUB_TOKEN` (`contents: write`) in its final step. No target-branch code runs in the credentialed job. |
...
7. Push to PR — CI compares read-only and reports failures with artifacts if baselines differ
8. If baselines changed, ask a maintainer to run the acceptance workflow (`workflow_dispatch`, `ref` = PR branch, `scope: kilo-vscode`) so the regenerated baselines are committed to the branch
