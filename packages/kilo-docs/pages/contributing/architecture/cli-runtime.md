...
| Non-goals | No mutation dispatch, `SessionOperation`, DB/EventV2/retention/cache authority, no cutover/G3 closure, no Gates B/C/D/P4.4 closure, no `session/messages`, no new protocol design. B6 is additive carrier evidence, not P4.4-G3/Gate closure. |

G3 remains **Active** and SDK authoritative. No cutover, no Gates B/C/D/P4.4 closure, no `session/messages` claim. B6 evidence is Darwin-local only; no Linux B6 CI/runtime PASS, no Windows/live Extension Host production proof. B6 is Active, not Complete.
