...
| Commit | `SessionForkDispatch.dispatch` in one `BEGIN IMMEDIATE`: source `SessionTable` exists → `InstanceStore.load` target resolution → `GenerationGate` barrier check (single continuous lease) → freshness `sessionRevision`/`configVersion` → create forked session (`SessionID.descending()`, `getForkedTitle`, filtered `MessageTable`/`PartTable` clone via `cloneMessageDataForFork`/`clonePartDataForFork`, sandbox inherit, `carryForkDiff`, `KiloSession.register`) → `INSERT session` → `EventV2.recordProjectedTx` `SessionV1.Event.Created` with explicit `Location` (canon dir + project worktree + workspace_id) → `INSERT session_operation` with `result_snapshot` → commit then `EventV2.notifyCommitted`. Exactly one forked session + one operation + one `Event` atomically; same-key replay returns validated persisted `Session.Info` with `makeRevision(existing.revision, currentConfig)` without new session. |
