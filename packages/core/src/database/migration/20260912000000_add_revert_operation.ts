...
export default {
  id: "20260912000000_add_revert_operation",
  up(tx) {
    return Effect.gen(function* () {
      ...
      const needsRebuild = tbl?.sql ? !tbl.sql.includes("'revert'") : false
      if (needsRebuild) {
        ...
        yield* tx.run(sql`
          CREATE TABLE "session_operation" (
            ...
            "op_kind" text NOT NULL CHECK("op_kind" IN ('prompt','provider','tool','permission','task','cancelQueued','sessionUpdate','fork','create','delete','revert','unrevert')),
