  ...
  up(tx) {
    return Effect.gen(function* () {
      ...
      if (needsRebuild) {
        ...
        // Create new table with full desired schema (including cancelQueued CHECK and all metadata columns)
        yield* tx.run(sql`
        ...
        // Use raw SQL for dynamic column list
      ...
      // Normal path: table already has correct CHECK, just ensure metadata columns/indexes
      const cols = yield* tx.all<{ name: string }>(sql`SELECT name FROM pragma_table_info('session_operation')`)
      const has = (name: string) => cols.some((c) => c.name === name)
