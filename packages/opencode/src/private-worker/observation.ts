...
export function assertObservationCapable(result: unknown): void {
  ...
  // illegal: capabilities contains null byte or empty key, or non-object value for observation/*
  ...
  // duplicate detection: if capabilities was supplied as array or duplicated keys, isPlainObject already fails array case
  // illegal value for observation version duplication would be caught via version check above
  ...
  try {
    for (const m of messages) {
      ...
      if (keys.length !== 2 || !keys.includes("info") || !keys.includes("parts"))
        throw new Error("messages returned invalid message shape")
      // Validate required shape without transforming: unknown legacy fields stay
      // on the returned original objects.
    ...
    // Chronological ASC (info.time.created then id) + cursor-anchor invariants.
    // Returns the original storage-stripped objects, not a re-decoded copy.
