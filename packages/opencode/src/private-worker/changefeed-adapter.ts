 ...
 * Reconstruction must not depend on it.
 */
  ...
  return {
    ...
    ack: async (cursor: number) => {
      try {
        ...
        // InvalidParams from validation already has code; preserve
        if (e instanceof Error && (e as { code?: number }).code === ErrorCode.InvalidParams) throw e
        // Negative cursor also becomes InvalidParams via controller's prior validation,
        // but Changefeed.ack treats it as CursorAheadError; map to InvalidParams for wire preservation
