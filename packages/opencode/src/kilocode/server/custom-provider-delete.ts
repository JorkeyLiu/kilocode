 ...
 *    writer ticket is acquired FIRST, then the deterministic global→project
 *    config flocks. Deletion never waits for the gate while holding a flock.
 *    A not-custom revalidation or prepared no-op aborts the ticket cleanly
 *    without a rebuild.
 ...
 *    clear the model cache LAST, then hand the ticket to exactly one
 *    ConfigRebuild.rebuildGlobal registration. The transaction events are
 *    DEFERRED into the result (LOCK-003): the HTTP handler emits them at the
 *    response acknowledgement boundary via HttpEffect.appendPreResponseHandler,
 *    so they are never observable before persistence, rebuild registration,
 *    and the success response value are all finalized. The response returns
 *    after persistence + registration, before generation drain (LOCK-007); the
 *    forked rebuild disposes old instances only after their readers and
 *    control leases drain.
 *
 ...
 *    reverse order while locks + ticket remain held — the exact auth file
...
export const execute = Effect.fn("CustomProviderDelete.execute")(function* (input: CustomProviderDeleteInput) {
  ...
  // writer ticket FIRST, then the deterministic global→project config flocks.
  // Revalidation/prepare happen only under this order, so deletion never waits
  // for the gate while holding a flock; a not-custom/no-op result aborts the
  // ticket cleanly without a rebuild (withWriteTicket releases on failure).
  return yield* withWriteTicket({
    acquire: gate.beginWriteGlobal(),
    run: (ticket) =>
      Effect.gen(function* () {
        // LOCK-001: the canonical discovery locks (global domain, then project
        // directory) are acquired before any target resolution. Targets are
        // resolved exactly once inside the lock body, and prepare/commit use
        // those exact resolved paths — never rediscovered under a different
        // lock, so the locked key is always the written path.

        // LOCK-005/007: pre-barrier instance identities captured after ticket
        ...
        const body = Effect.gen(function* () {
          // LOCK-002: each global/project target is resolved exactly once under
          // the discovery locks, before prepare/commit.
          ...
          // LOCK-006: compensate in reverse order while locks + ticket are held.
        ...
        // LOCK-001/007: discovery locks are acquired after the ticket; the
        // response is returned after persistence + rebuild registration,
        // before generation drain; exactly one rebuild is registered for the
        // changed mutation.
        const value = yield* config.withLock(
          KilocodeConfig.configDiscoveryGlobalKey(),
          discoveryProjectKey ? config.withLock(discoveryProjectKey, body) : body,
        )
        return { changed: true, value, rebuild: ConfigRebuild.rebuildGlobal(ticket, olds) }
      }),
