...
// Shared serve/full-CLI bootstrap implementation. Both `src/index.ts` (via
// `kilocode/cli/setup`) and the lightweight `src/serve-entry.ts` use this
// namespace directly so auth/config/telemetry semantics stay identical
// without copying logic. This module must not import any yargs command
// tree — only the services bootstrap/shutdown need.
export namespace KiloBootstrap {
