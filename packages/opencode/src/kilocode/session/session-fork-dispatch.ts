...
export function validateRequest(raw: unknown): SessionForkRequest {
  ...
  if (typeof c.directory !== "string" || !isAbsolute(c.directory as string))
  ...
  if (c.parentSessionId !== null && c.parentSessionId !== undefined)
    throw new Error("parentSessionId must be null for fork")
    ...
    const dispatch = Effect.fn("SessionForkDispatch.dispatch")(function* (raw: unknown) {
      ...
      const inner = Effect.gen(function* () {
        ...
        if (existing) {
          ...
          if (existing.outcome === "succeeded") {
            ...
            return buildFailed(
              ...
              false,
              false,
              makeRevision(yield* readRevOmit(sessionId), yield* readCfgOmit(canonDir)),
            )
          }
          const curRev = yield* readRevOmit(sessionId)
          const curCfg = yield* readCfgOmit(canonDir)
          ...
          if (forkedId) {
            if (ownedBase) {
              const okStorage = yield* ForkSeam.failCleanupStorage
                ? Effect.gen(function* () {
                    ForkSeam.capturedCleanupWarnings.push({
                      target: forkedId as string,
                      cause: "injected cleanup storage failure",
                    })
                    yield* Effect.logWarning("durable fork cleanup base storage failed", {
                      target: forkedId,
                      cause: "injected cleanup storage failure",
                    })
                ...
                : Effect.promise(() => storageRuntime.runPromise((s) => s.remove(baseKey(forkedId as string)))).pipe(
                    ...
                    Effect.catch((e) =>
                      Effect.logWarning("durable fork cleanup base storage failed", {
                        target: forkedId,
                        cause: String(e),
                      }).pipe(Effect.as(false as const)),
                    ),
                    Effect.catchDefect((e) =>
                      Effect.logWarning("durable fork cleanup base storage defect", {
                        target: forkedId,
                        cause: String(e),
                      }).pipe(Effect.as(false as const)),
                    ),
              ...
              const okFs = yield* ForkSeam.failCleanupFs
                ? Effect.gen(function* () {
                    ForkSeam.capturedCleanupWarnings.push({
                      target: forkedId as string,
                      cause: "injected cleanup fs failure",
                    })
                    yield* Effect.logWarning("durable fork cleanup base fs failed", {
                      target: forkedId,
                      cause: "injected cleanup fs failure",
                    })
                  }).pipe(Effect.as(false as const))
                : Effect.promise(() =>
                    fs.rm(storageFileForKey(baseKey(forkedId as string), Global.Path.data), { force: true }),
                  ).pipe(
                    Effect.map(() => true as const),
                    Effect.catch((e) =>
                      Effect.logWarning("durable fork cleanup base fs failed", {
                        target: forkedId,
                        cause: String(e),
                      }).pipe(Effect.as(false as const)),
                    ),
                    Effect.catchDefect((e) =>
                      Effect.logWarning("durable fork cleanup base fs defect", {
                        target: forkedId,
                        cause: String(e),
                      }).pipe(Effect.as(false as const)),
                    ),
            ...
            if (ownedDiff) {
              const okStorage = yield* ForkSeam.failCleanupStorage
                ? Effect.gen(function* () {
                    ...
                    yield* Effect.logWarning("durable fork cleanup diff storage failed", {
                      target: forkedId,
                      cause: "injected cleanup storage failure",
                    })
                  }).pipe(Effect.as(false as const))
                : Effect.promise(() =>
                    storageRuntime.runPromise((s) => s.remove(["session_diff", forkedId as string])),
                  ).pipe(
                    Effect.map(() => true as const),
                    Effect.catch((e) =>
                      Effect.logWarning("durable fork cleanup diff storage failed", {
                        target: forkedId,
                        cause: String(e),
                      }).pipe(Effect.as(false as const)),
                    ),
                    Effect.catchDefect((e) =>
                      Effect.logWarning("durable fork cleanup diff storage defect", {
                        target: forkedId,
                        cause: String(e),
                      }).pipe(Effect.as(false as const)),
                    ),
              ...
              const okFs = yield* ForkSeam.failCleanupFs
                ? Effect.gen(function* () {
                    ...
                    yield* Effect.logWarning("durable fork cleanup diff fs failed", {
                      target: forkedId,
                      cause: "injected cleanup fs failure",
                    })
                  }).pipe(Effect.as(false as const))
                : Effect.promise(() =>
                    fs.rm(storageFileForKey(["session_diff", forkedId as string], Global.Path.data), { force: true }),
                  ).pipe(
                    Effect.map(() => true as const),
                    Effect.catch((e) =>
                      Effect.logWarning("durable fork cleanup diff fs failed", {
                        target: forkedId,
                        cause: String(e),
                      }).pipe(Effect.as(false as const)),
                    ),
                    Effect.catchDefect((e) =>
                      Effect.logWarning("durable fork cleanup diff fs defect", {
                        target: forkedId,
                        cause: String(e),
                      }).pipe(Effect.as(false as const)),
                    ),
            ...
            if (ownedSandbox) {
              const okRemove = yield* Effect.promise(() =>
                SandboxStore.remove(canonDir, forkedId as unknown as SessionID),
              ).pipe(
                Effect.map(() => true as const),
                Effect.catch((e) =>
                  Effect.logWarning("durable fork cleanup sandbox remove failed", {
                    target: forkedId,
                    cause: String(e),
                  }).pipe(Effect.as(false as const)),
                ),
                Effect.catchDefect((e) =>
                  Effect.logWarning("durable fork cleanup sandbox remove defect", {
                    target: forkedId,
                    cause: String(e),
                  }).pipe(Effect.as(false as const)),
                ),
              )
              const okEvict = yield* Effect.sync(() =>
                SandboxPolicy.evict(canonDir, forkedId as unknown as SessionID),
              ).pipe(
                Effect.map(() => true as const),
                Effect.catch((e) =>
                  Effect.logWarning("durable fork cleanup sandbox evict failed", {
                    target: forkedId,
                    cause: String(e),
                  }).pipe(Effect.as(false as const)),
                ),
                Effect.catchDefect((e) =>
                  Effect.logWarning("durable fork cleanup sandbox evict defect", {
                    target: forkedId,
                    cause: String(e),
                  }).pipe(Effect.as(false as const)),
                ),
            ...
            yield* Effect.sync(() => KiloSession.clearPlatformOverride(forkedId as string)).pipe(
              Effect.catch((e) =>
                Effect.logWarning("durable fork cleanup KiloSession clear failed", {
                  target: forkedId,
                  cause: String(e),
                }).pipe(Effect.asVoid),
              ),
              Effect.catchDefect((e) =>
                Effect.logWarning("durable fork cleanup KiloSession clear defect", {
                  target: forkedId,
                  cause: String(e),
                }).pipe(Effect.asVoid),
              ),
        ...
        const txResult: SessionForkResult = yield* Effect.gen(function* () {
          ...
          const existingSandbox = yield* Effect.promise(() =>
            SandboxStore.read(canonDir, newId as unknown as SessionID),
          ).pipe(
          ...
          const localForDiff = yield* Effect.promise(() =>
            storageRuntime.runPromise((s) =>
              s
                .read<any>(["session_diff", String(sessionId)])
                .pipe(Effect.catchIf(isNotFound, () => Effect.succeed([] as any))),
            ),
          ).pipe(
          ...
          const baseForDiff = yield* Effect.promise(() =>
            storageRuntime.runPromise((s) => cumulativeSessionDiff(s, sessionId, localForDiff as any)),
          )
              ...
              if (isClaimedWriteError(err)) {
                ...
                const ok = yield* ForkSeam.failCleanupFs
                  ? Effect.gen(function* () {
                      ...
                      yield* Effect.logWarning("durable fork claimed cleanup failed", {
                        target: claimed.target,
                        cause: "injected cleanup fs failure",
                      })
                    }).pipe(Effect.as(false as const))
                  : Effect.promise(() => claimed.handle.cleanup()).pipe(
                      ...
                      Effect.catch((e) =>
                        Effect.logWarning("durable fork claimed cleanup failed", {
                          target: claimed.target,
                          cause: String(e),
                        }).pipe(Effect.as(false as const)),
                      ),
                      Effect.catchDefect((e) =>
                        Effect.logWarning("durable fork claimed cleanup defect", {
                          target: claimed.target,
                          cause: String(e),
                        }).pipe(Effect.as(false as const)),
                      ),
                ...
                if (ok) ownedBase = false
                  ...
                  yield* Effect.logWarning("durable fork claimed file retained (cleanup failed)", {
                    target: claimed.target,
                    cause: msg,
                  })
                const curRev = yield* readRevOmit(sessionId)
            ...
            if (secondOutcome._tag === "fail") {
              ...
              if (isClaimedWriteError(err)) {
                ...
                const ok2 = yield* ForkSeam.failCleanupFs
                  ? Effect.gen(function* () {
                      ForkSeam.capturedCleanupWarnings.push({
                        target: claimed2.target,
                        cause: "injected cleanup fs failure",
                      })
                      yield* Effect.logWarning("durable fork claimed second cleanup failed", {
                        target: claimed2.target,
                        cause: "injected cleanup fs failure",
                      })
                    }).pipe(Effect.as(false as const))
                  : Effect.promise(() => claimed2.handle.cleanup()).pipe(
                      Effect.map((v) => v as boolean),
                      Effect.catch((e) =>
                        Effect.logWarning("durable fork claimed second cleanup failed", {
                          target: claimed2.target,
                          cause: String(e),
                        }).pipe(Effect.as(false as const)),
                      ),
                      Effect.catchDefect((e) =>
                        Effect.logWarning("durable fork claimed second cleanup defect", {
                          target: claimed2.target,
                          cause: String(e),
                        }).pipe(Effect.as(false as const)),
                      ),
                ...
                if (ok2) ownedDiff = false
                  ...
                  yield* Effect.logWarning("durable fork claimed second file retained", {
                    target: claimed2.target,
                    cause: msg2,
                  })
                ...
                const okFirst = yield* ForkSeam.failCleanupFs
                  ? Effect.gen(function* () {
                      ...
                      yield* Effect.logWarning("durable fork second diff partial cleanup failed", {
                        target: String(firstKey),
                        cause: "injected cleanup fs failure",
                      })
                    }).pipe(Effect.as(false as const))
                  : Effect.promise(() => fs.rm(storageFileForKey(firstKey, Global.Path.data), { force: true })).pipe(
                      Effect.map(() => true as const),
                      Effect.catch((e) =>
                        Effect.logWarning("durable fork second diff partial cleanup failed", {
                          target: String(firstKey),
                          cause: String(e),
                        }).pipe(Effect.as(false as const)),
                      ),
                      Effect.catchDefect((e) =>
                        Effect.logWarning("durable fork second diff partial cleanup defect", {
                          target: String(firstKey),
                          cause: String(e),
                        }).pipe(Effect.as(false as const)),
                      ),
          ...
          if (ForkSeam.failSandboxWrite) {
            ...
            return buildFailed(
              req,
              "internal",
              "injected sandbox write failure",
              false,
              false,
              makeRevision(curRev, curCfg),
            ) as unknown as SessionForkResult
          ...
          if (parentSnap) {
            ...
            const sandboxOutcome = yield* Effect.gen(function* () {
              yield* Effect.promise(() =>
                SandboxStore.writeExclusive(canonDir, newId as unknown as SessionID, nextSnap),
              )
              ownedSandbox = true
            }).pipe(
              Effect.map(() => ({ _tag: "ok" as const })),
            ...
            if (sandboxOutcome._tag === "fail") {
              ...
              if (isClaimedWriteError(err)) {
                ...
                const okSb = yield* ForkSeam.failCleanupFs
                  ? Effect.gen(function* () {
                      ForkSeam.capturedCleanupWarnings.push({
                        target: claimedSb.target,
                        cause: "injected cleanup fs failure",
                      })
                      yield* Effect.logWarning("durable fork sandbox claimed cleanup failed", {
                        target: claimedSb.target,
                        cause: "injected cleanup fs failure",
                      })
                    }).pipe(Effect.as(false as const))
                  : Effect.promise(() => claimedSb.handle.cleanup()).pipe(
                      Effect.map((v) => v as boolean),
                      Effect.catch((e) =>
                        Effect.logWarning("durable fork sandbox claimed cleanup failed", {
                          target: claimedSb.target,
                          cause: String(e),
                        }).pipe(Effect.as(false as const)),
                      ),
                      Effect.catchDefect((e) =>
                        Effect.logWarning("durable fork sandbox claimed cleanup defect", {
                          target: claimedSb.target,
                          cause: String(e),
                        }).pipe(Effect.as(false as const)),
                      ),
                ...
                if (okSb) ownedSandbox = false
                  ...
                  yield* Effect.logWarning("durable fork sandbox claimed file retained", {
                    target: claimedSb.target,
                    cause: msgSb,
                  })
          ...
          const txOut: TxOut = yield* db.transaction(
            (tx) =>
              Effect.gen(function* () {
                if (ForkSeam.failTxAfterFs)
                  return yield* Effect.fail(new Error("injected transaction failure after filesystem writes"))
                const already = yield* SessionOperation.getSessionForkByIdempotencyHashTx(
                    ...
                    if (hasSnap && !persisted)
                      return {
                        result: buildFailed(
                          ...
                          false,
                          makeRevision(
                            (already as unknown as { revision: number }).revision,
                            yield* readCfgOmit(canonDir),
                          ),
                        ),
                      } as unknown as TxOut
                    if (persisted) {
                      ...
                      return {
                        result: buildSucceeded(
                          ...
                          makeRevision(persistedRev, yield* readCfgOmit(canonDir)),
                        ),
                      } as unknown as TxOut
                    }
                    return {
                ...
                const authRevEither = yield* (
                  SessionRevision.getTx(tx as unknown as typeof db, sessionId) as unknown as Effect.Effect<
                    unknown,
                    unknown,
                    unknown
                  >
                ...
                if ((authRevEither as unknown as { _tag: string })._tag === "Left") {
                  return {
                    result: buildFailed(
                      req,
                      "internal",
                      "revision read failed inside tx",
                      false,
                      false,
                      makeRevision(undefined, undefined),
                    ),
                ...
                if (
                  authRev !== undefined &&
                  req.context.sessionRevision !== undefined &&
                  req.context.sessionRevision < authRev
                ) {
                  const cfgInsideEither = yield* (
                    ...
                    { _tag: "Right"; right: number | undefined } | { _tag: "Left"; left: unknown }
                  >
                  const latestCfg = isLeft(cfgInsideEither)
                    ? undefined
                    : (rightValue(cfgInsideEither) as number | undefined)
                  return {
                    result: buildFailed(
                      req,
                      "stale",
                      ...
                      false,
                      false,
                      makeRevision(authRev, latestCfg ?? effectiveConfigBeforeTx),
                    ),
                  } as unknown as TxOut
                }
                const cfgInsideEither = yield* (
                ...
                if ((cfgInsideEither as unknown as { _tag: string })._tag === "Left") {
                  return {
                    result: buildFailed(
                      ...
                      false,
                      false,
                      makeRevision(authRev ?? actualSessionRev, undefined),
                    ),
                  } as unknown as TxOut
                }
                ...
                if (
                  ...
                  return {
                    result: buildFailed(
                      ...
                      false,
                      false,
                      makeRevision(revForStale, effectiveInside),
                    ),
                  } as unknown as TxOut
                }
                ...
                if (!src)
                  return {
                    result: buildFailed(
                      req,
                      "session.not_found",
                      `source session not found ${sessionId}`,
                      false,
                      false,
                ...
                const msgRows = yield* tx
                  ...
                  .all()
                  .pipe(Effect.orDie)
                // filterMessagesForFork(msgRows - kernel delegation)
                const filteredRows = filterMessagesForFork(
                  msgRows as unknown as Array<{ id: string }>,
                  messageId as unknown as string | null,
                ) as typeof msgRows
                const model = resolveForkModelAtCheckpoint({
                  sourceModel: (
                    src as unknown as { model: { id: string; providerID: string; variant?: string } | null }
                  ).model as unknown as { id: string; providerID: string; variant?: string } | null,
                  checkpointId: messageId as unknown as string | null,
                  orderedMessages: msgRows.map((r) => ({
                    id: r.id as unknown as string,
                    role: (r.data as Record<string, unknown>).role as string,
                    model: (r.data as Record<string, unknown>).model,
                  })),
                }) as unknown
                ...
                for (const row of filteredRows) {
                  ...
                  yield* tx
                    .insert(MessageTable)
                    .values(newMsgRow as unknown as typeof MessageTable.$inferInsert)
                    .run()
                    .pipe(Effect.orDie)
                  ...
                  for (const prow of partRows) {
                    const prepared = KiloSession.prepareForkedPart(
                      prow.data as unknown as Parameters<typeof KiloSession.prepareForkedPart>[0],
                    )
                    if (!prepared) continue
                    const mapped = clonePartDataForFork(
                      prepared as unknown as import("@/session/message-v2").MessageV2.Part,
                    ...
                    yield* tx
                      .insert(PartTable)
                      .values(partInsert as unknown as typeof PartTable.$inferInsert)
                      .run()
                      .pipe(Effect.orDie)
                  }
                }
                ...
                const opRec = yield* SessionOperation.insertSessionForkSucceededTx(
                  tx as unknown as typeof db,
                  sessionId,
                  record,
                  meta,
          ...
          if (side) {
            yield* Effect.sync(() => KiloSession.register({ id: side.newId, parentID: side.parentID })).pipe(
              Effect.catch((e: unknown) =>
                Effect.sync(() =>
                  log.warn("fork post-commit register failed", {
              ...
              Effect.catchDefect((e: unknown) =>
                Effect.sync(() =>
                  log.warn("fork post-commit register defect", { error: String(e), newId: side.newId }),
                ),
              ),
            )
          }
