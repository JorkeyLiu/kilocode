...
export class SessionUpdateDispatchService extends Context.Service<
  ...
  Effect.gen(function* () {
    ...
    const dispatch = Effect.fn("SessionUpdateDispatch.dispatch")(function* (raw: unknown) {
      ...
      const inner = Effect.gen(function* () {
        ...
        const existing = yield* SessionOperation.getSessionUpdateByIdempotencyHash(db, sessionId, hash).pipe(
          Effect.orDie,
        )
        if (existing) {
          ...
          if (existing.outcome === "succeeded") {
            const hasSnapshot = SessionOperation.hasSnapshot(
              existing as unknown as SessionOperation.SessionUpdateRecord,
            )
            ...
            const info = yield* db
              ...
              .pipe(
                Effect.map((row) =>
                  row ? Session.fromRow(row as unknown as Parameters<typeof Session.fromRow>[0]) : undefined,
                ),
              )
        ...
        const txInnerResult: SessionUpdateResult = yield* Effect.gen(function* () {
          ...
          if (isLeft(revEither))
            return buildFailed(
              ...
              false,
              makeRevision(undefined, undefined),
            ) as unknown as SessionUpdateResult
          const cfgEither = yield* readConfigVer(canonDir)
          if (isLeft(cfgEither))
            return buildFailed(
              req,
          ...
          const reserveResult: ReserveResult = yield* db.transaction(
            (tx) =>
              Effect.gen(function* () {
                const already = yield* SessionOperation.getSessionUpdateByIdempotencyHashTx(
                ...
                if (opExists)
                  return {
                    ...
                    existing: opExists as unknown as SessionOperation.SessionUpdateRecord,
                  }
                const authRevEffect = SessionRevision.getTx(tx as unknown as typeof db, sessionId)
                const authRevEither = yield* (
                  authRevEffect as unknown as Effect.Effect<unknown, unknown, unknown>
                ).pipe(
                  Effect.map((v) => ({ _tag: "Right" as const, right: v as number | undefined })),
                  Effect.catch((e: unknown) => Effect.succeed({ _tag: "Left" as const, left: e })),
                  Effect.catchDefect((e: unknown) => Effect.succeed({ _tag: "Left" as const, left: e })),
                ...
                const inserted = yield* SessionOperation.insertSessionUpdateSucceededTx(
                  tx as unknown as typeof db,
                  sessionId,
                  record,
                  meta,
                ...
                const rawSnap = hasSnapshot
                  ? (inserted as unknown as { resultSnapshot: unknown }).resultSnapshot
                  : undefined
                ...
                if (hasSnapshot) {
                  ...
                  const row = yield* tx
                    ...
                    .pipe(Effect.orDie)
                  if (row)
                    infoForEvent = Session.fromRow(
                      row as unknown as Parameters<typeof Session.fromRow>[0],
                    ) as unknown as Session.Info
                }
                if (!infoForEvent) {
                ...
                const loc = new Location.Info({
                  ...
                  ...(sessionRow.workspace_id
                    ? { workspaceID: sessionRow.workspace_id as unknown as WorkspaceV2.ID }
                    : {}),
                  project: {
                    id: Project.ID.make(sessionRow.project_id),
                    directory: AbsolutePath.make(projectWorktree),
                  },
                })
                const event = yield* events.recordProjectedTx(
                  tx as unknown as typeof db,
                  SessionV1.Event.Updated,
                  { sessionID: sessionId, info: infoForEvent as unknown as Session.Info },
            ...
            if (existingReplay.outcome === "succeeded") {
              ...
              if (hasSnap && !persisted) {
                const revision = makeRevision(
                  (existingReplay as unknown as { revision: number }).revision ?? latestRev ?? actualSessionRev,
                  latestCfg ?? effectiveConfigBeforeTx,
                )
                return buildFailed(req, "internal", "invalid persisted snapshot", false, false, revision)
              }
              if (persisted) {
                ...
                const revision = makeRevision(
                  persistedRev ?? latestRev ?? actualSessionRev,
                  latestCfg ?? effectiveConfigBeforeTx,
                )
                return buildSucceeded(req, persisted as unknown as Session.Info, revision)
              }
          ...
          const newRev =
            persistedRev ?? (isLeft(revEither3) ? undefined : (rightValue(revEither3) as number | undefined))
          ...
          yield* events.notifyCommitted(reserveResult.event).pipe(
            Effect.catch((cause: unknown) =>
              Effect.sync(() =>
                log.warn("notifyCommitted failed", { error: cause instanceof Error ? cause.message : String(cause) }),
              ),
            ),
            Effect.catchDefect((defect: unknown) =>
              Effect.sync(() =>
                log.warn("notifyCommitted defect", {
