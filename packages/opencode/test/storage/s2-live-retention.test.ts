        ...
        Effect.gen(function* () {
          ...
          try {
            ...
            yield* Effect.gen(function* () {
              ...
              const sortedBefore = [...feedBefore].sort((a, b) => a.revision - b.revision)
              expect(sortedBefore[0].kind).toBe("changed")
              expect(sortedBefore[0].revision).toBe(0)
              expect(sortedBefore[1].kind).toBe("changed")
              expect(sortedBefore[1].revision).toBe(beforeRev)
              ...
              const feedAfter = yield* db
                ...
                .all()
                .pipe(Effect.orDie)
              expect(feedAfter.length).toBe(3)
              const sortedAfter = [...feedAfter].sort((a, b) => a.revision - b.revision)
              expect(sortedAfter[0].kind).toBe("changed")
              expect(sortedAfter[0].revision).toBe(0)
              expect(sortedAfter[1].kind).toBe("changed")
              expect(sortedAfter[1].revision).toBe(beforeRev)
              expect(sortedAfter[2].kind).toBe("deleted")
              expect(sortedAfter[2].revision).toBe(beforeRev + 1)
        ...
        Effect.gen(function* () {
          ...
          try {
            ...
            yield* Effect.gen(function* () {
              ...
              expect(Accounting.artifactBytes(storageDir)).toBe(
                Accounting.artifactBytesForSession(storageDir, fresh.id),
              )
        ...
        Effect.gen(function* () {
          ...
          try {
            ...
            yield* Effect.gen(function* () {
              ...
              expect(
                existsSync(join(storageDir, "snapshot"))
                  ? readdirSync(join(storageDir, "snapshot")).length === 0
                  : true,
              ).toBe(true)
              expect(existsSync(join(dir, "snapshot"))).toBe(false)
