...
The root AGENTS.md **Architecture documentation completion gate** also applies before claiming completion and before commits. Test-only changes never produce checker signals (`.test`/`.spec` files are exempt at every tier), so the expected outcome is normally the concise no-signal report; when tests encode lifecycle or config-contract expectations (see Config save lifecycle tests below), keep the canonical docs they assert in sync.
