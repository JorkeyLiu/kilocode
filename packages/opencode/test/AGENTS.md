...
When a change may have architecture impact, or when preparing a commit, the root AGENTS.md **Architecture documentation completion gate** applies as conditional guidance: assess the diff, update the canonical docs when the architecture meaning changed, and run the checker as guidance. Pure investigation and ordinary no-impact tasks need no check and no report. Test-only changes never produce checker signals (`.test`/`.spec` files are exempt at every tier), so the expected outcome is normally the concise no-signal report; when tests encode lifecycle or config-contract expectations (see Config save lifecycle tests below), keep the canonical docs they assert in sync.
