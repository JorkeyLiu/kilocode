...
describe("R12 classification", () => {
  it("each domain with unknown error → <domain>.unknown", () => {
    ...
    // tool with unknown shape as string should be tool.unknown; Error yields tool.failed so use string for unknown
...
describe("R12 normalization boundary", () => {
  it("error input → outcome failed, caps, time preserved, message wins", () => {
    ...
    // detail is built from name+message+status; make message long to exceed 1000
    ...
    // detail should be capped at 1000 + ellipsis = 1001
...
describe("R12 redaction", () => {
  ...
  it("depth cap 8", () => {
    ...
    // should not throw and deepest should be truncated
...
describe("R12 envelope", () => {
  it("version === 1.0 and payload is project-tier plain object", () => {
    const rec: FailureRecord = {
      ...
      outcome: "failed",
      code: "c",
      message: "apiKey=sk-123",
      time: 1,
      detail: "d",
      stack: "s",
    }
    // envelope normalizes via normalizeRecord before select, so direct raw records are scrubbed before projection; payload is project-tier only and secrets are redacted idempotently.
    ...
    // payload is exactly project fields
    const expectedKeys = new Set(["opId", "outcome", "code", "message"])
    // when no cancel, those 4
  ...
  it("direct raw quoted and bearer forms are scrubbed before projection — quoted/bearer regression", () => {
    const raw: FailureRecord = {
      ...
      outcome: "failed",
      code: "provider.unknown",
      // quoted keys with double/single quotes, bearer with space/Bearer prefix and equals form
    ...
    expect(msg).toContain("password=[redacted]")
    expect(msg).toContain("credential=[redacted]")
    // bearer collapses to authorization=[redacted] regardless of Bearer prefix
    expect(msg).toContain("authorization=[redacted]")
    expect(env.payload["detail"]).toBeUndefined()
    expect(env.payload["stack"]).toBeUndefined()
    // caps/idempotent still hold via helper path
    const normalized = normalizeRecord(raw)
    expect(normalized.message).toBe(msg)
    ...
    // already-redacted input remains idempotent
