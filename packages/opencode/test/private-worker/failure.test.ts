...
describe("R12 envelope", () => {
  it("version === 1.0 and payload is project-tier plain object", () => {
    ...
    // normalize already redacts, but envelope should contain redacted message from record as is; test with direct record containing secret to see envelope redaction? Envelope uses select, not redact, so message in payload is as in record (already redacted if via normalize). Test that envelope payload has no detail/stack and has redacted message when record came from normalize.
