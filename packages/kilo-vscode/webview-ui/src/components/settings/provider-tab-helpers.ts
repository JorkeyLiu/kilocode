...
export type PrimarySlot = "account" | "edit" | "apiKey" | "chatgpt" | "anaconda" | "placeholder"
...
/**
 * Pure predicate: is the provider the Kilo Gateway?
 */
export function isKiloProvider(item: Provider): boolean {
  return item.id === KILO_PROVIDER_ID
}
...
/** Build the Configured providers list. */
export function buildConfiguredList(
  ...
  const list = Array.from(ids).map((id) => resolveConfiguredProvider(id, allProviders, configProvider))
