/**
 * Extension Host E2E runner — child task open tab-order case + in-session
 * per-agent variant memory case.
 *
 ...
 *   7. on the harness's `child-phase1-done` marker, re-selects A so the harness
 *      can re-click the button and prove an already-open child is focused
 *      WITHOUT reordering, then writes `<scratch>/child-phase2-ready`,
 *   8. on the harness's `child-phase2-done` marker, seeds the variant scenario:
 *        - opens variant session D and loads a transcript whose user message
 *          pins `kilo/e2e-probe` (recovery) and `code` (recovered agent),
 *        - calls the fixture bridge's `provisionVariantModel` LAST, which
 *          injects `kilo/e2e-probe` with variants low/medium/high into the
 *          served catalog and pins it as the per-agent model for every backend
 *          agent (the models.dev snapshot ships no variant-bearing models), so
 *          the synthetic providersLoaded stays the final provider message the
 *          webview processes and switching agents keeps the variant model,
 *        - re-seeds the session list so D survives,
 *      then writes `<scratch>/variant-ready` so the harness can drive the real
 *      ThinkingSelector + ModeSwitcher and assert per-agent variant restore,
 *   9. exits when the harness writes `<scratch>/done`, so VS Code exits 0.
 *
 * Scenario selection (KILO_E2E_SCENARIO, set by the harness):
 *   - (unset) | all      => child-task-order then variant-memory, sequentially,
 *                           in this one VS Code lifecycle,
 ...
 *   child-phase2-done, variant-ready). `ready`, `done`, `runner-done` are
 *   process-level harness gates, not scenario state.
 *
...
export async function run(): Promise<void> {
  ...
  if (!runChild && !runVariant) {
    throw new Error(
      `probe runner: unknown KILO_E2E_SCENARIO "${scenario}". ` +
        "Supported values: all | child-task-order | variant-memory (default: all)",
    )
  }
