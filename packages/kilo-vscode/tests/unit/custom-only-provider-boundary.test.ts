...
describe("custom-only anaconda boundary", () => {
  ...
  it("fails closed without touching the Anaconda backend", async () => {
    ...
    expect(src).not.toContain("AnacondaDesktopBridge")
    expect(src).not.toMatch(/client\.anacondaDesktop/)
    expect(src).not.toMatch(/anacondaDesktop\.handle/)
    expect(existsSync(resolve(ROOT, "src/anaconda-desktop/bridge.ts"))).toBe(false)
    host.dispose()
  })
})
