...
/**
 * R1 private transport scaffold: JSON-RPC 2.0 over child-process stdio with
 * standard Content-Length framing, initialize handshake, bounded stderr,
 * and EOF/process-exit lifecycle ownership.
 *
 * This peer is reusable on both extension (client) and worker (server) sides.
 * It does NOT implement R9 observation/changefeed or R11-R14 Failure/Outcome.
 *
 * Lifecycle:
 *  - open: framing and dispatch active
 *  - closed: EOF or process exit or explicit dispose; pending requests rejected
 *
 * Transport owns EOF/exit: when the readable closes or the child exits, peer
 * transitions to closed and rejects all pending requests with -32603.
 */
...
export interface PeerOptions {
  ...
  // Optional child handle for exit-owned lifecycle (extension side). When
  // provided, peer listens for exit/close and transitions to closed.
  child?: ChildProcess
...
export class JsonRpcPeer {
  ...
  /**
   * Send a JSON-RPC request and await its response. Rejects if peer is closed
  ...
  private bindReader(): void {
    ...
    this.reader.on("data", onData as unknown as (c: unknown) => void)
    this.reader.on("end", onEnd)
    this.reader.on("close", onClose as unknown as () => void)
    this.reader.on("error", onError as unknown as () => void)
    // Keep handles for unbind
  ...
  private handleBody(body: string): void {
    ...
    if (!parsed.ok) {
      // Parse error: JSON invalid — respond with -32700 and null id per spec.
      this.sendRaw(makeError(null, ErrorCode.ParseError, "Parse error"))
      return
    }
    const obj = parsed.value
    // Distinguish responses (have id and result/error) vs requests/notifications (have method)
    if (isResponse(obj)) {
