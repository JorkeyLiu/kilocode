...
// P4.4-G3-B6 production evidence: ServerManager → real `kilo serve` child →
// fd3/fd4 → same AppLayer `session/get` (parity-only, read-only).
// SDK HTTP (`client.session.get`) remains the sole authority; the private
// path is non-blocking parity diagnostics only (no state/event/error impact,
// no mutation/durable operation, no `session/messages`). Darwin + Linux run;
// other platforms skip per B4 convention. Windows/live Extension Host
...
// no B5 test edits). Limitations: fresh-session comparison covers
// id/directory/title only (volatile extras excluded by comparator); failure
// parity covers missing-session 404 class; isolation covers same-session
// cross-directory scope_mismatch (SDK get is directory-agnostic, private
// binds directory); no replacement-epoch or live Extension Host proof.
...
describe("ServerManager → real kilo serve → fd3/fd4 → SessionGet parity-only production", () => {
  test.skipIf(process.platform !== "darwin" && process.platform !== "linux")(
    "covers initialize session/get capability + same-directory private get vs SDK authoritative + missing-session failure parity + cross-directory isolation + invalid-wire exclusion + fail-closed cleanup",
    async () => {
      ...
      // Diagnostic only (not a freshness gate): a binary predating the B6
      ...
      console.log(`[b6-prod] bin mtime age ms=${mtimeAge} size=${stat.size}`)
      ...
      // restoration) with the B5 integration files (reuse only).
      const release = await acquireB5GlobalLock()
      try {
        workspaceRaw = fs.mkdtempSync(path.join(os.tmpdir(), "kilo-prod-b6-ws-"))
        workspace = fs.realpathSync(workspaceRaw)
        peerDirRaw = fs.mkdtempSync(path.join(os.tmpdir(), "kilo-prod-b6-peer-"))
        peerDir = fs.realpathSync(peerDirRaw)
        storage = fs.mkdtempSync(path.join(os.tmpdir(), "kilo-prod-b6-storage-"))
        xdgData = fs.mkdtempSync(path.join(os.tmpdir(), "kilo-prod-b6-xdg-"))
        ...
        // 2. Initialize private peer against fd3/fd4; require session/get.
        ...
        if (Array.isArray(caps)) expect(caps.includes("session/get")).toBeTrue()
        else {
          const str = JSON.stringify(caps ?? "")
          expect(str.includes("session/get") || str.includes("get")).toBeTrue()
        }
        expect(peer.hasCapability("session/get")).toBeTrue()
        ...
        const created = await client.session.create({ directory: workspace, title: "prod-b6-source" })
        ...
        // 4. SDK get authoritative: full Session.Info read via SDK.
        const sdkGet = await client.session.get({ sessionID: source.id, directory: workspace })
        expect((sdkGet as unknown as { error?: unknown }).error).toBeUndefined()
        const sdkData = (sdkGet as unknown as { data: Record<string, unknown> }).data
        expect(isRecord(sdkData)).toBeTrue()
        expect(sdkData.id).toBe(source.id)
        expect(typeof sdkData.directory).toBe("string")
        expect(typeof sdkData.title).toBe("string")
        expect(sdkData.title).toBe("prod-b6-source")

        // 5. Private same-directory get over fd3/fd4; explicit valid outcome.
        const token = crypto.randomUUID().replace(/-/g, "").slice(0, 8)
        const opId = canonicalGetOpId(source.id, token)
        const req = {
          v: 1 as const,
        ...
        expect(priv.op).toBe("session/get")
        ...
        if (priv.status === "succeeded") {
          ...
          expect(isRecord(priv.data.session)).toBeTrue()
          expect(opId.startsWith(`get:${source.id}:`)).toBeTrue()
          expect(() => validateGetResult(priv as unknown, req as unknown as never)).not.toThrow()
        }

        // 6. Parity diagnostics only: SDK agreement on id/directory/title, no authority change.
        const parity = compareGetParity(priv, { data: sdkData } as unknown as never)
        ...
        const beforeStr = JSON.stringify(source)
        expect(JSON.stringify((sdkGet as unknown as { data: unknown }).data)).toBe(beforeSdk)
        const parityConn = {
          isPrivateAvailable: () => peer!.isAvailable(),
          privateGet: (r: unknown) => peer!.privateGet(r as never),
          privateGetWithHandle: (r: unknown) => peer!.privateGetWithHandle(r as never),
          privateGetOutcomeWithHandle: (r: unknown) => peer!.privateGetOutcomeWithHandle(r as never),
          getPrivateEpoch: () => peer!.getEpoch(),
        }
        const ret = observeSessionGetParityDetached(
          parityConn as unknown as Parameters<typeof observeSessionGetParityDetached>[0],
          sdkGet as unknown as Parameters<typeof observeSessionGetParityDetached>[1],
          source.id,
          workspace,
          3000,
        )
        ...
        expect(JSON.stringify((sdkGet as unknown as { data: unknown }).data)).toBe(beforeSdk)
        expect(JSON.stringify(source)).toBe(beforeStr)
        await new Promise((r) => setTimeout(r, 300))
        expect(JSON.stringify((sdkGet as unknown as { data: unknown }).data)).toBe(beforeSdk)

        // 8. Failure parity: missing session fails on both sides with 404 class.
        const missingId = `ses_b6missing${crypto.randomUUID().replace(/-/g, "").slice(0, 8)}`
        const sdkMissing = await client.session.get({ sessionID: missingId, directory: workspace })
        ...
        const missOpId = canonicalGetOpId(missingId, missToken)
        const missReq = {
          v: 1 as const,
          requestId: crypto.randomUUID(),
          opId: missOpId,
        ...
        const missOutcome = await peer.privateGetOutcomeWithHandle(missReq as unknown as never).promise
        ...
        if (missOutcome.result.status === "failed") {
          ...
          expect(() => validateGetResult(missOutcome.result as unknown, missReq as unknown as never)).not.toThrow()
        }
        const missParity = compareGetParity(missOutcome.result, sdkMissing as unknown as never)
        expect(missParity.divergence).toBeNull()

        // 9. Cross-directory isolation: same sessionId via a different
        // directory binds strictly — private fails scope_mismatch while SDK
        // get (directory-agnostic) still succeeds, proving directory binding.
        const xToken = crypto.randomUUID().replace(/-/g, "").slice(0, 8)
        const xOpId = canonicalGetOpId(source.id, xToken)
        const xReq = {
          v: 1 as const,
          requestId: crypto.randomUUID(),
          opId: xOpId,
        ...
        const xOutcome = await peer.privateGetOutcomeWithHandle(xReq as unknown as never).promise
        expect(xOutcome.kind).toBe("valid")
        if (xOutcome.kind !== "valid") throw new Error(`expected valid wire outcome for cross-directory get, got ${xOutcome.kind}`)
        ...
        const sdkCross = await client.session.get({ sessionID: source.id, directory: peerDir })
        expect((sdkCross as unknown as { error?: unknown }).error).toBeUndefined()
        const xParity = compareGetParity(xOutcome.result, sdkCross as unknown as never)
        expect(xParity.divergence).toContain("status-mismatch")

        // 10. Direct-helper invalid-wire exclusion: normalizePrivateGetWire
        ...
        const wire = normalizePrivateGetWire(malformed, req as unknown as never)
        expect(wire.kind).toBe("invalid")
        if (wire.kind !== "invalid") throw new Error("expected invalid wire outcome for non-object session id")

        // 11. Private unavailable fallback while SDK stays authoritative.
        peer.dispose()
        expect(peer.isAvailable()).toBeFalse()
        const getAfterPeerClose = await client.session.get({ sessionID: source.id, directory: workspace })
        expect((getAfterPeerClose as unknown as { error?: unknown }).error).toBeUndefined()
        let threw = false
        try {
