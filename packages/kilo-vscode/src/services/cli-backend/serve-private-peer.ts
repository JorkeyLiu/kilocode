...
import {
  ...
  ConfigUiDefaultsValidationError,
...
export class ServePrivatePeer {
  ...
  privatePtyUpdateOutcomeWithHandle(req: ServePrivatePtyUpdateRequest): {
    id: number
    promise: Promise<PrivatePtyUpdateWireOutcome>
    cancel: (msg?: string) => boolean
  } {
    validatePtyUpdateRequest(req)
    if (this.disposed) throw new Error("Peer disposed")
    if (!this.available || !this.peer || this.peer.getState() !== "open") throw new Error("Private peer unavailable")
    if (!this.hasCapability(req.op)) throw new Error(`Private peer missing ${req.op} capability`)
    const currentEpoch = this.opts.epoch
    const peerAtCall = this.peer
    const op = req.op
    return requestPtyUpdateOutcome(
      peerAtCall as unknown as import("./serve-private-pty").PtyRawTransport,
      {
        isStale: () => this.isStaleHandle(peerAtCall, currentEpoch),
        isClosed: (e) => this.isClosedHandle(peerAtCall, currentEpoch, e),
  ...
  /** Atomic handle: allocates id synchronously and returns exact id for timeout cancellation ownership.
   * Resolved values are always strictly valid pty-update results; invalid wire rejects
   * with PrivatePtyValidationError and never resolves as a normal result.
   */
  privatePtyUpdateWithHandle(req: ServePrivatePtyUpdateRequest): {
    id: number
    promise: Promise<ServePrivatePtyUpdateResult>
    cancel: (msg?: string) => boolean
  } {
    const outcome = this.privatePtyUpdateOutcomeWithHandle(req)
    const promise = (async (): Promise<ServePrivatePtyUpdateResult> => {
      const wire = await outcome.promise
      if (wire.kind === "invalid") throw new PrivatePtyValidationError(wire.detail)
      return wire.result
    })()
  ...
  privatePtyRemoveOutcomeWithHandle(req: ServePrivatePtyRemoveRequest): {
    id: number
    promise: Promise<PrivatePtyRemoveWireOutcome>
    cancel: (msg?: string) => boolean
  } {
    validatePtyRemoveRequest(req)
    if (this.disposed) throw new Error("Peer disposed")
    if (!this.available || !this.peer || this.peer.getState() !== "open") throw new Error("Private peer unavailable")
    if (!this.hasCapability(req.op)) throw new Error(`Private peer missing ${req.op} capability`)
    const currentEpoch = this.opts.epoch
    const peerAtCall = this.peer
    const op = req.op
    return requestPtyRemoveOutcome(
      peerAtCall as unknown as import("./serve-private-pty").PtyRawTransport,
      {
        isStale: () => this.isStaleHandle(peerAtCall, currentEpoch),
        isClosed: (e) => this.isClosedHandle(peerAtCall, currentEpoch, e),
  ...
  /** Atomic handle: allocates id synchronously and returns exact id for timeout cancellation ownership.
   * Resolved values are always strictly valid pty-remove results; invalid wire rejects
   * with PrivatePtyValidationError and never resolves as a normal result.
   */
  privatePtyRemoveWithHandle(req: ServePrivatePtyRemoveRequest): {
    id: number
    promise: Promise<ServePrivatePtyRemoveResult>
    cancel: (msg?: string) => boolean
  } {
    const outcome = this.privatePtyRemoveOutcomeWithHandle(req)
    const promise = (async (): Promise<ServePrivatePtyRemoveResult> => {
      const wire = await outcome.promise
      if (wire.kind === "invalid") throw new PrivatePtyValidationError(wire.detail)
      return wire.result
    })()
  ...
  private failedSandboxSupport(req: SandboxSupportContractRequest, code: string, msg: string): SandboxSupportResult {
    return {
      v: 1,
      requestId: req.requestId,
      op: "sandbox/support",
  ...
  async privateSandboxSupport(req: SandboxSupportContractRequest): Promise<SandboxSupportResult> {
    const handle = this.privateSandboxSupportOutcomeWithHandle(req)
    const outcome = await handle.promise
    if (outcome.kind === "invalid") throw new SandboxSupportValidationError(outcome.detail)
    return outcome.result
  }

  /**
   * Internal normalized handle for the sessionless `sandbox/support`
   * private-first observation. Resolves the discriminated wire outcome so
   * invalid wire is an explicit `{ kind: "invalid" }` value consumed before
   * any SDK fallback, never a normal result. Observation identity is
   * `requestId` only.
   ...
   * `transportUnknown`. Generic rejection (unknown failure code) fails closed
   * validation and stays fallback-eligible; only validated domain terminals
   * (`validation.failed`/`scope_mismatch`) close.
   */
  privateSandboxSupportOutcomeWithHandle(req: SandboxSupportContractRequest): {
    ...
    validateSandboxSupportContractRequest(req)
    if (this.disposed) throw new Error("Peer disposed")
    if (!this.available || !this.peer || this.peer.getState() !== "open") {
      throw new Error("Private peer unavailable")
    }
    if (!this.hasCapability("sandbox/support")) {
      throw new Error("Private peer missing sandbox/support capability")
    }
    const currentEpoch = this.opts.epoch
    const peerAtCall = this.peer
    const { id, promise: rawPromise } = peerAtCall.requestWithId("sandbox/support", req)
    const promise = (async (): Promise<SandboxSupportWireOutcome> => {
      let raw: unknown
      try {
        raw = (await rawPromise) as unknown
      } catch (e: unknown) {
        if (this.isClosedHandle(peerAtCall, currentEpoch, e))
          return { kind: "valid", result: makeSandboxSupportAmbiguous(req, true) }
        const { code, msg } = this.parseFailedInfo(e)
        return { kind: "valid", result: this.failedSandboxSupport(req, code, msg) }
      }
      const wire = normalizePrivateSandboxSupportWire(raw, req)
      if (wire.kind === "invalid") return wire
      if (isSettledSandboxSupportResult(wire.result, req)) return wire
      if (this.isStaleHandle(peerAtCall, currentEpoch))
        return { kind: "valid", result: makeSandboxSupportAmbiguous(req, true) }
      return wire
    })()
  ...
  async privateConfigUiDefaults(req: ConfigUiDefaultsContractRequest): Promise<ConfigUiDefaultsResult> {
    const handle = this.privateConfigUiDefaultsOutcomeWithHandle(req)
    const outcome = await handle.promise
    if (outcome.kind === "invalid") throw new ConfigUiDefaultsValidationError(outcome.detail)
    return outcome.result
  }

  /**
   * Internal normalized handle for the read-only `config/ui-defaults`
   * private-first observation. Resolves the discriminated wire outcome so
   * invalid wire is an explicit `{ kind: "invalid" }` value consumed before
   * any SDK fallback, never a normal result. Observation identity is
   * `requestId` only.
  ...
  privateConfigUiDefaultsOutcomeWithHandle(req: ConfigUiDefaultsContractRequest): {
    id: number
    promise: Promise<ConfigUiDefaultsWireOutcome>
    cancel: (msg?: string) => boolean
  } {
    validateConfigUiDefaultsContractRequest(req)
    if (this.disposed) throw new Error("Peer disposed")
    if (!this.available || !this.peer || this.peer.getState() !== "open") {
      throw new Error("Private peer unavailable")
    }
    if (!this.hasCapability("config/ui-defaults")) {
      throw new Error("Private peer missing config/ui-defaults capability")
    }
    const currentEpoch = this.opts.epoch
    const peerAtCall = this.peer
    const { id, promise: rawPromise } = peerAtCall.requestWithId("config/ui-defaults", req)
    const promise = (async (): Promise<ConfigUiDefaultsWireOutcome> => {
      let raw: unknown
      try {
        raw = (await rawPromise) as unknown
      } catch (e: unknown) {
        if (this.isClosedHandle(peerAtCall, currentEpoch, e))
          return { kind: "valid", result: makeConfigUiDefaultsAmbiguous(req, true) }
        const { code, msg } = this.parseFailedInfo(e)
        return { kind: "valid", result: this.failedConfigUiDefaults(req, code, msg) }
      }
      const wire = normalizePrivateConfigUiDefaultsWire(raw, req)
      if (wire.kind === "invalid") return wire
      if (isSettledConfigUiDefaultsResult(wire.result, req)) return wire
      if (this.isStaleHandle(peerAtCall, currentEpoch))
        return { kind: "valid", result: makeConfigUiDefaultsAmbiguous(req, true) }
      return wire
    })()
  ...
  async privateQuestionList(req: QuestionListContractRequest): Promise<QuestionListResult> {
    const handle = this.privateQuestionListWithHandle(req)
    const outcome = await handle.promise
    if (outcome.kind === "invalid") throw new Error(outcome.detail)
    return outcome.result
  }

  private failedQuestionList(req: QuestionListContractRequest, code: string, msg: string): QuestionListResult {
    return {
      v: 1,
      requestId: req.requestId,
      opId: req.opId,
  ...
  privateQuestionListWithHandle(req: QuestionListContractRequest): {
    id: number
    promise: Promise<QuestionListWireOutcome>
    cancel: (msg?: string) => boolean
  } {
    ...
    if (!this.hasCapability("question/list")) {
      throw new Error("Private peer missing question/list capability")
    }
    const currentEpoch = this.opts.epoch
    const peerAtCall = this.peer
    const { id, promise: rawPromise } = peerAtCall.requestWithId("question/list", req)
    const promise = (async (): Promise<QuestionListWireOutcome> => {
      let raw: unknown
      try {
        raw = (await rawPromise) as unknown
      } catch (e: unknown) {
        ...
        const { code, msg } = this.parseFailedInfo(e)
        return { kind: "valid", result: this.failedQuestionList(req, code, msg) }
      }
      if (this.isStaleHandle(peerAtCall, currentEpoch))
        return { kind: "valid", result: makeQuestionListAmbiguous(req, true) }
