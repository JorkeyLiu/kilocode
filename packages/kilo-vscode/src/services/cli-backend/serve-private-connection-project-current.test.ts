...
import { DeferredProjectCurrent } from "./serve-private-project-current"

function makeService(): KiloConnectionService {
  return new KiloConnectionService({} as never)
}

function projectReq(token = "tok1") {
...
function succeededResult(req: ReturnType<typeof projectReq>, data: unknown = {}) {
  return {
    v: 1,
...
function fakePeer(outcome: unknown, caps = ["project/current"]) {
  return {
    dispose: () => {},
    isAvailable: () => true,
...
describe("project-current connection-service owner", () => {
  test("unavailable peer throws without touching the transport", () => {
    const service = makeService()
    const req = projectReq()
    expect(() => service.privateProjectCurrentOutcomeWithHandle(req as never)).toThrow("Private peer unavailable")
    service.dispose()
  })
  ...
  test("missing capability throws fail-closed", () => {
    ...
    const req = projectReq()
    expect(() => service.privateProjectCurrentOutcomeWithHandle(req as never)).toThrow(
  ...
  test("current epoch passes the normalized outcome through", async () => {
    const service = makeService()
    const req = projectReq()
    installPeer(service, fakePeer({ kind: "valid", result: succeededResult(req, { vcs: "git" }) }))
    const handle = service.privateProjectCurrentOutcomeWithHandle(req as never)
  ...
  test("invalid wire passes through before any comparator", async () => {
    ...
    const outcome = await service.privateProjectCurrentOutcomeWithHandle(req as never).promise
  ...
  test("deferred project-current observer respects epoch lifecycle without retention", () => {
    const service = makeService()
    // Null epoch: impossible registration, no retention, never fires.
    ;(service as unknown as Record<string, unknown>).privateEpoch = null
    ;(service as unknown as Record<string, unknown>).privateFailedGetEpoch = null
    let fires = 0
    const noop1 = service.addDeferredProjectCurrentObserver("/tmp", undefined, () => {
      fires += 1
    })
    ...
    const noop2 = service.addDeferredProjectCurrentObserver("/tmp", undefined, () => {
      fires += 1
    })
  ...
  test("deferred project-current observer dedupes per epoch+directory without retention on failure", () => {
    const service = makeService()
    ;(service as unknown as Record<string, unknown>).privateEpoch = 7
    ;(service as unknown as Record<string, unknown>).privateFailedGetEpoch = null
    let fires = 0
    const unsub1 = service.addDeferredProjectCurrentObserver("/tmp", undefined, () => {
      fires += 1
    })
    const unsub2 = service.addDeferredProjectCurrentObserver("/tmp", undefined, () => {
      fires += 1
    })
  ...
  test("deferred project-current owner key is opaque and collision-safe across tuples", () => {
    const store = new DeferredProjectCurrent(new Set<() => void>())
  ...
  test("dispose clears deferred project-current observers", () => {
    const service = makeService()
    ;(service as unknown as Record<string, unknown>).privateEpoch = 7
    ;(service as unknown as Record<string, unknown>).privateFailedGetEpoch = null
    ;(service as unknown as Record<string, unknown>).privateAvailable = false
    service.addDeferredProjectCurrentObserver("/tmp", undefined, () => {})
    const store = (
      service as unknown as {
        deferredProjectCurrent: { key: (e: number | null, d: string, w: string | undefined) => string }
      }
    ).deferredProjectCurrent
  ...
  test("negotiation failure clears failed epoch keys and admits the next epoch", () => {
    ...
    let oldFires = 0
    service.addDeferredProjectCurrentObserver("/tmp/old-epoch", undefined, () => {
      oldFires += 1
    })
    ...
    rec.privateAvailable = false
    let newFires = 0
    service.addDeferredProjectCurrentObserver("/tmp/new-epoch", undefined, () => {
      newFires += 1
    })
    expect(rec.privateAvailableListeners.size).toBe(1)
  ...
  test("stale peer replacement clears only the replaced epoch", () => {
    ...
    rec.privateFailedGetEpoch = null
    rec.privateAvailable = false
    service.addDeferredProjectCurrentObserver("/tmp/stale-epoch", undefined, () => {})
    rec.privateEpoch = 8
    ...
    rec.privateFailedGetEpoch = null
    rec.privateAvailable = false
    service.addDeferredProjectCurrentObserver("/tmp/other", undefined, () => {})
  ...
  test("superseded init clears the stale epoch without touching the current epoch", () => {
    ...
    rec.privateFailedGetEpoch = null
    rec.privateAvailable = false
    service.addDeferredProjectCurrentObserver("/tmp/current", undefined, () => {})
    expect(rec.privateAvailableListeners.size).toBe(1)
    const stale = { dispose: () => {}, getEpoch: () => 7 }
    rec.privateEpoch = 7
    service.addDeferredProjectCurrentObserver("/tmp/stale", undefined, () => {})
    expect(rec.privateAvailableListeners.size).toBe(2)
    rec.privateEpoch = 9
  ...
  test("clearForEpoch drops only the old epoch opaque keys", () => {
    ...
    const store = new DeferredProjectCurrent(listeners)
    let oldFires = 0
    let newFires = 0
