...
import { canonicalSuggestionListOpId, validateSuggestionListContractRequest } from "./serve-private-suggestion-list-contract"
...
function makeReq(over: Record<string, unknown> = {}) {
  const opId = canonicalSuggestionListOpId("tok1")
  return {
    v: 1 as const,
    requestId: "r1",
    opId,
...
function setupPeer(epoch: number, caps: unknown = ["suggestion/list"]) {
...
describe("suggestion/list private peer", () => {
  test("malformed request fails closed before transport", () => {
    const bad = makeReq({ payload: { filter: {} } })
    expect(() => validateSuggestionListContractRequest(bad)).toThrow()
    const { chan, peer } = setupPeer(5)
    try {
      expect(() => peer.privateSuggestionListWithHandle(bad as never)).toThrow()
  ...
  test("peer unavailable/disposed fails closed", () => {
    ...
    const req = makeReq()
    expect(() => peer.privateSuggestionListWithHandle(req as never)).toThrow("Private peer unavailable")
    ;(peer as unknown as Record<string, unknown>).available = true
    peer.dispose()
    expect(() => peer.privateSuggestionListWithHandle(req as never)).toThrow()
    chan.backendPeer.dispose()
    chan.clientReader.destroy()
    chan.clientWriter.destroy()
  })

  test("capability gating requires suggestion/list", () => {
  ...
  test("handle resolves valid success with zero mutation", async () => {
    ...
    const { clientReader, clientWriter, backendPeer } = createLinkedChannel((method) => {
      expect(method).toBe("suggestion/list")
      return success
    })
    ...
    ;(peer as unknown as Record<string, unknown>).capabilities = ["suggestion/list"]
    try {
      const outcome = await peer.privateSuggestionListWithHandle(req as never).promise
      expect(outcome.kind).toBe("valid")
      if (outcome.kind === "valid") expect(outcome.result.status).toBe("succeeded")
      const direct = await peer.privateSuggestionList(req as never)
  ...
  test("invalid wire normalizes to invalid outcome and direct throws", async () => {
    ...
    try {
      ...
      await expect(peer.privateSuggestionList(req as never)).rejects.toThrow()
    } finally {
      peer.dispose()
      backendPeer.dispose()
      clientReader.destroy()
  ...
  test("transport failure maps to ambiguous with exact cancel", async () => {
    ...
    ;(peer as unknown as Record<string, unknown>).capabilities = ["suggestion/list"]
    try {
      const handle = peer.privateSuggestionListWithHandle(req as never)
