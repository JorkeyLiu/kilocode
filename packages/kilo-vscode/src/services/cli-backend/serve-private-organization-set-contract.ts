// Private-first `kilo/organization/set` mutation contract (production).
// Request is strictly `{v:1,requestId,opId,op:"kilo/organization/set",
// idempotencyKey,context:{directory,workspace?},payload:{organizationId}}`
// with `opId === organization-set:<token>` (single token, non-empty, no
// colon, no path material) and `idempotencyKey === opId`. The directory is
// routing/connection identity only (absolute path, canonical validation
// server-side, never an auth scope). The payload carries the stable
// organization identity (`string | null`: `null` is the personal account);
// the caller must reuse the same `organizationId` for the private attempt
// and the exactly-one same-tuple SDK `client.kilo.organization.set`
// fallback.
//
// Source facts:
// - Private entry: `packages/opencode/src/kilocode/organization-set-private.ts`
//   (`kilo/organization/set` FD op, strict validation, no drain-control/
//   `InstanceRef` lane, no scope mismatch, the same global cold mutation as
//   the HTTP `kilo.organization.set` route via shared `organizationSetMutate`
//   over `invalidateAfterProviderAuthChange` with credential preservation and
//   `clearModesCache` in mutate, redacted terminal failures).
// - Consumer: `packages/kilo-vscode/src/kilo-provider/organization-set-privatefirst.ts`
...
//   `client.kilo.organization.set` fallback. Repeating the same
//   `organizationId` is a safe overwrite, so an ambiguous private outcome may
//   safely repeat via the SDK fallback.
...
export const ORGANIZATION_SET_OP = "kilo/organization/set" as const
...
export function parseOrganizationSetOpId(opId: string): { token: string } {
  ...
  if (segs.length !== 2 || segs[0] !== "organization-set" || segs[1]!.length === 0)
    throw new TypeError("opId must be organization-set:<token> with nonempty colon-free token")
  const token = segs[1]!
  if (!pathless(token)) throw new TypeError("opId must be organization-set:<token> with nonempty colon-free token")
  return { token }
}

export interface OrganizationSetContractRequest {
  v: 1
  requestId: string
  opId: string
  op: typeof ORGANIZATION_SET_OP
...
function validateIds(raw: Record<string, unknown>): void {
  ...
  if (raw.op !== ORGANIZATION_SET_OP) throw new Error("op must be kilo/organization/set")
  if (!present(raw.idempotencyKey)) throw new Error("idempotencyKey must be non-empty string")
  if (raw.idempotencyKey !== raw.opId) throw new Error("idempotencyKey must equal opId for organization-set contract")
...
function validatePayload(raw: unknown): void {
  ...
  const organizationId = (raw as Record<string, unknown>).organizationId
...
export function validateOrganizationSetContractRequest(raw: unknown): OrganizationSetContractRequest {
  ...
  return raw as unknown as OrganizationSetContractRequest
}
...
export type OrganizationSetResult =
  | {
      v: 1
      requestId: string
      opId: string
...
export function makeOrganizationSetAmbiguous(req: OrganizationSetContractRequest, transportUnknown = true): OrganizationSetResult {
  const out: OrganizationSetResult = {
    v: 1,
...
// Redacted failure shape ({code,message,retryable} only). Organization,
// path, directory, credential, and transport echo keys are rejected so
...
// invalid wire and fails closed to the SDK fallback. `unauthorized` preserves
// the backend's real auth-not-found/non-oauth/set-failure semantics (the HTTP
// route's `Unauthorized`); it is not collapsed into `internal`.
const FAILURE_CODES = new Set(["validation.failed", "unauthorized", "internal"])

export function validateOrganizationSetFailure(raw: unknown): OrganizationSetFailure {
...
export type OrganizationSetWireOutcome = { kind: "valid"; result: OrganizationSetResult } | { kind: "invalid"; detail: string }
...
export function validateOrganizationSetResult(raw: unknown, req: OrganizationSetContractRequest): OrganizationSetResult {
  ...
  if (raw.op !== ORGANIZATION_SET_OP) throw new Error("op mismatch")
  ...
  if (status === "succeeded") {
    ...
    return raw as unknown as OrganizationSetResult
  }
  if (status === "failed") {
    ...
    const failure = validateOrganizationSetFailure(rec.failure)
    const outFailure = validateOrganizationSetFailure(outRec.failure)
...
export function isSettledOrganizationSetResult(result: unknown, req: OrganizationSetContractRequest): boolean {
