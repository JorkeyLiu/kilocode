// Private-first `auth/remove` mutation contract (production).// Request is strictly `{v:1,requestId,opId,op:"auth/remove",
// idempotencyKey,context:{directory,workspace?},payload:{providerID}}` with
// `opId === auth-remove:<token>` (single token, non-empty, no colon, no path
// material) and `idempotencyKey === opId`. The directory is routing/connection
// identity only (absolute path, canonical validation server-side, never an
// auth scope). The payload carries the stable provider identity; the caller
// must reuse the same `providerID` for the private attempt and the exactly-one
// same-tuple SDK `client.auth.remove` fallback.
//
// Source facts:
// - Private entry: `packages/opencode/src/kilocode/auth-remove-private.ts`
//   (`auth/remove` FD op, strict validation, no drain-control/`InstanceRef`
//   lane, no scope mismatch, the same global cold mutation as the HTTP
//   `auth.remove` route via `invalidateAfterProviderAuthChange` over
//   `Auth.remove`, redacted terminal failures).
// - Consumer: `packages/kilo-vscode/src/kilo-provider/auth-remove-privatefirst.ts`
...
//   `client.auth.remove` fallback. `Auth.remove` is idempotent, so an ambiguous
//   private outcome may safely repeat via the SDK fallback.
...
export const AUTH_REMOVE_OP = "auth/remove" as const
...
export function parseAuthRemoveOpId(opId: string): { token: string } {
  ...
  if (segs.length !== 2 || segs[0] !== "auth-remove" || segs[1]!.length === 0)
    throw new TypeError("opId must be auth-remove:<token> with nonempty colon-free token")
  const token = segs[1]!
  if (!pathless(token)) throw new TypeError("opId must be auth-remove:<token> with nonempty colon-free token")
  return { token }
}

export interface AuthRemoveContractRequest {
  v: 1
  requestId: string
  opId: string
  op: typeof AUTH_REMOVE_OP
...
function validateIds(raw: Record<string, unknown>): void {
  ...
  if (raw.op !== AUTH_REMOVE_OP) throw new Error("op must be auth/remove")
  if (!present(raw.idempotencyKey)) throw new Error("idempotencyKey must be non-empty string")
  if (raw.idempotencyKey !== raw.opId) throw new Error("idempotencyKey must equal opId for auth-remove contract")
...
function validatePayload(raw: unknown): void {
  ...
  if ((providerID as string).includes("\0")) throw new Error("payload.providerID must not contain null bytes")
}

export function validateAuthRemoveContractRequest(raw: unknown): AuthRemoveContractRequest {
  ...
  return raw as unknown as AuthRemoveContractRequest
}
...
export function makeAuthRemoveAmbiguous(req: AuthRemoveContractRequest, transportUnknown = true): AuthRemoveResult {
  const out: AuthRemoveResult = {
    v: 1,
...
// Redacted failure shape ({code,message,retryable} only). Provider, path,
// directory, credential, and transport echo keys are rejected so responses
// cannot carry sensitive material.
...
const FAILURE_CODES = new Set(["validation.failed", "internal"])

export function validateAuthRemoveFailure(raw: unknown): AuthRemoveFailure {
...
export type AuthRemoveWireOutcome = { kind: "valid"; result: AuthRemoveResult } | { kind: "invalid"; detail: string }
...
export function validateAuthRemoveResult(raw: unknown, req: AuthRemoveContractRequest): AuthRemoveResult {
  ...
  if (raw.op !== AUTH_REMOVE_OP) throw new Error("op mismatch")
  ...
  if (status === "succeeded") {
    ...
    return raw as unknown as AuthRemoveResult
  }
  if (status === "failed") {
    ...
    const failure = validateAuthRemoveFailure(rec.failure)
    const outFailure = validateAuthRemoveFailure(outRec.failure)
...
export function isSettledAuthRemoveResult(result: unknown, req: AuthRemoveContractRequest): boolean {
