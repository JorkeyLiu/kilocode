...
import { isSettledProviderCatalogResult, makeProviderCatalogAmbiguous } from "./serve-private-provider-catalog-contract"
import type {
  ProviderCatalogContractRequest,
  ProviderCatalogWireOutcome,
} from "./serve-private-provider-catalog-contract"

type Handle = { id: number; promise: Promise<ProviderCatalogWireOutcome>; cancel: (msg?: string) => boolean }
...
// Epoch-guarded `provider/catalog` observation handle (observation identity is
...
export function providerCatalogHandle(deps: Deps, req: ProviderCatalogContractRequest): Handle {
  const at = deps.epoch
  const peer = deps.peer
  if (!peer || !deps.live || !peer.isAvailable()) throw new Error("Private peer unavailable")
  if (!peer.hasCapability("provider/catalog")) throw new Error("Private peer missing provider/catalog capability")
  const handle = (peer as unknown as {
    privateProviderCatalogOutcomeWithHandle: (r: ProviderCatalogContractRequest) => {
      id: number
      promise: Promise<ProviderCatalogWireOutcome>
      cancel: (msg?: string) => boolean
    }
  }).privateProviderCatalogOutcomeWithHandle(req)
  const promise = handle.promise.then((outcome) => {
    if (outcome.kind === "valid" && isSettledProviderCatalogResult(outcome.result, req)) return outcome
    if (at !== null && deps.epoch !== at) return { kind: "valid", result: makeProviderCatalogAmbiguous(req, true) } as ProviderCatalogWireOutcome
    if (deps.peer !== peer) return { kind: "valid", result: makeProviderCatalogAmbiguous(req, true) } as ProviderCatalogWireOutcome
    return outcome
  })
