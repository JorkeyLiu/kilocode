...
function makeHarness(opts: {
  ...
  const connection = {
    ...
    privateMessagesOutcomeWithHandle: (req: Record<string, unknown>) => {
      ...
      return {
        ...
        promise: Promise.resolve({
          kind: "valid",
          result: { v: 1, requestId: req.requestId, opId: req.opId, op: "session/messages", idempotencyKey: req.idempotencyKey, status: "ambiguous", outcome: { type: "ambiguous", time: 1 }, accepted: false, transportUnknown: true },
        }),
      }
    },
    privateMessagesWithHandle: (req: Record<string, unknown>) => {
      ...
      return {
        id: parityCalls.length,
        promise: Promise.resolve({ v: 1, requestId: (req as Record<string, unknown>).requestId, opId: (req as Record<string, unknown>).opId, op: "session/messages", idempotencyKey: (req as Record<string, unknown>).idempotencyKey, status: "ambiguous", outcome: { type: "ambiguous", time: 1 }, accepted: false, transportUnknown: true }),
      }
    },
    getClient: () => client,
...
describe("paged messages private-first", () => {
  ...
  it("private terminal not_found/scope_mismatch authoritative: no SDK, no parity", async () => {
    for (const status of ["not_found", "scope_mismatch"] as const) {
      ...
      try {
        await fetchMessagePage(h.client as never, { sessionID: h.sid, workspaceDir: h.dir, limit: 2 }, h.connection as never, h.reader as never)
      } catch (e) {
        threw = e
      }
  ...
  it("private malformed falls back exactly once with bounded warn and no raw data", async () => {
    ...
    try {
      const h = makeHarness({ privateMessages: async () => ({ v: "1.0", status: "found", messages: [{ info: { id: "bad" }, parts: [] }] }) })
      const page = await fetchMessagePage(h.client as never, { sessionID: h.sid, workspaceDir: h.dir, limit: 2 }, h.connection as never, h.reader as never)
  ...
  it("private InternalError/MethodNotFound/transport/closed each fallback exactly once", async () => {
    ...
    for (const err of cases) {
      ...
      console.warn = (...a: unknown[]) => warns.push(a)
      try {
        const h = makeHarness({ privateMessages: async () => { throw err } })
        const page = await fetchMessagePage(h.client as never, { sessionID: h.sid, workspaceDir: h.dir, limit: 2 }, h.connection as never, h.reader as never)
        expect(page.items).toHaveLength(1)
        expect(h.privateCalls).toHaveLength(1)
        expect(h.sdkCalls).toHaveLength(1)
  ...
  it("disabled/not-started/legacy reader fallback silently to SDK once", async () => {
    ...
    await fetchMessagePage(h2.client as never, { sessionID: h2.sid, workspaceDir: h2.dir, limit: 2 }, h2.connection as never, h2.reader as never)
    expect(h2.privateCalls).toHaveLength(0)
  })

  it("limit=0 full read stays SDK-only even when private enabled", async () => {
    const h = makeHarness({})
    const page = await fetchMessagePage(h.client as never, { sessionID: h.sid, workspaceDir: h.dir, limit: 0 }, h.connection as never, h.reader as never)
  ...
  it("cursor passthrough: before maps to private cursor input", async () => {
    ...
    await fetchMessagePage(h.client as never, { sessionID: h.sid, workspaceDir: h.dir, limit: 5, before }, h.connection as never, h.reader as never)
  ...
  it("out-of-order private page falls back exactly once", async () => {
    const h = makeHarness({
      privateMessages: async (input) => privateFound(input.sessionId, [userMsg("msg_2", 200, input.sessionId), userMsg("msg_1", 100, input.sessionId)]),
    })
    const warns: unknown[][] = []
    const orig = console.warn
    console.warn = (...a: unknown[]) => warns.push(a)
  ...
  it("doLoadMessages private found preserves messagesLoaded then drainSince and stale checks", async () => {
    ...
    const origDrain = (anyP["streams"] as { drainSince: (id: string, since: number, keep?: unknown) => void }).drainSince.bind(anyP["streams"])
    ;(anyP["streams"] as { drainSince: (id: string, since: number, keep?: unknown) => void }).drainSince = (id: string, since: number, keep?: unknown) => {
      drains.push({ id, since })
      return origDrain(id, since, keep as never)
    }
    const ok = await (h.provider as unknown as { doLoadMessages: (id: string, opts: unknown, strict: boolean) => Promise<boolean> }).doLoadMessages(h.sid, { mode: "replace" }, false)
    expect(ok).toBeTrue()
    expect(h.privateCalls).toHaveLength(1)
    expect(h.sdkCalls).toHaveLength(0)
    const loaded = posts.find((p) => (p as Record<string, unknown>).type === "messagesLoaded") as Record<string, unknown>
  ...
  it("SDK fallback preserves original AbortSignal", async () => {
    ...
    const h = makeHarness({
      privateMessages: async () => { throw Object.assign(new Error("boom"), { code: ErrorCode.InternalError }) },
      sdkMessages: async (_p: unknown, o: unknown) => {
        seen.push(o as { signal?: AbortSignal })
        return { data: [userMsg("msg_2", 200)], error: undefined, response: { status: 200, headers: { get: () => null } } }
      },
    })
    ...
    const h = makeHarness({
      privateMessages: async (input) => {
        if (first && input.sessionId === "ses_aaa111") {
          return new Promise<unknown>((res) => { resolveFirst = res })
        }
        return privateFound(input.sessionId, [userMsg("msg_1", 100, input.sessionId)])
      },
    })
  ...
  it("validator exercises real wiring: found valid, terminal exact, malformed throws", async () => {
    ...
    let threw = false
    try {
      validatePrivateMessagesResult({ v: "1.0", status: "found", messages: [{ info: { id: "bad" }, parts: [] }] }, 2, SECRET_SES)
    } catch {
      threw = true
    }
  ...
  it("part sessionID mismatch triggers bounded fallback, never authoritative", async () => {
    ...
    try {
      ...
      const page = await fetchMessagePage(h.client as never, { sessionID: h.sid, workspaceDir: h.dir, limit: 1 }, h.connection as never, h.reader as never)
      expect(page.items).toHaveLength(1)
      expect(h.privateCalls).toHaveLength(1)
      expect(h.sdkCalls).toHaveLength(1)
      expect(warns.some((w) => String(w[0]).includes("[Kilo Messages]"))).toBeTrue()
  ...
  it("per-page fallback: double private failure yields two private attempts, one SDK per page, one parity, merged fill", async () => {
    const assistant = { info: { id: "msg_old", sessionID: SECRET_SES, role: "assistant", time: { created: 100 } }, parts: [] }
    ...
    const h = makeHarness({
      ...
      sdkMessages: async (p: unknown) => {
        sdkN += 1
        sdkParams.push(p)
        if (sdkN === 1) return { data: [assistant], error: undefined, response: { status: 200, headers: { get: () => "cur-1" } } }
  ...
  it("production delegate shape reaches provider private-first with no SDK", async () => {
    ...
    const productionReader = {
      ...
      list: (input: { directory: string; archived?: boolean; cursor?: string; limit?: number }) => fakeObservation.list() as Promise<unknown>,
    ...
    const provider = new KiloProvider(
      ...
      h.connection,
      undefined,
      { projectDirectory: h.dir, privateSessionReader: productionReader } as unknown as Parameters<typeof KiloProvider>[3],
    ) as unknown as Record<string, unknown> & KiloProvider
    ...
    const reader = (provider as unknown as { privateSessionReader: { messages: (i: unknown) => Promise<unknown> } }).privateSessionReader
    expect(typeof reader.messages).toBe("function")
    const page = await fetchMessagePage(h.client as never, { sessionID: h.sid, workspaceDir: h.dir, limit: 1 }, h.connection as never, reader as never)
    expect(page.items).toHaveLength(1)
