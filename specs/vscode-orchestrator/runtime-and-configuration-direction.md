...
| R9 Observation/hydration implementation details (snapshot/event handshake; revision scope/ordering/idempotency; ephemeral-fact retention) | Bounded implementation decision; the normative contract (section 7.1) fixes the one-owner and lifecycle-convergence constraints but not the wire schema, event sourcing, polling, timer subsystems, or retention. R9 may use a bounded derived changefeed/outbox for reconnect deltas; it is not authoritative history, is not required for reconstruction, and is eligible for automatic truncation after authoritative hydration state exists (ADR-0005; storage spec section 5.2) | P4.2 (P4.2a storage sub-boundary first, then the P4.2b wire/schema freeze; the private-worker observation surface must not ship without it). Required by P4.2 only; not a P1-P3 blocker | Open — added 2026-08-13; protocol foundation 2026-08-22 (audited 2026-08-22): versioned envelope `1.0` with `observation/snapshot`, `observation/read`, `observation/ack`, `observation/subscribe` + `observation/changed` (payload-free `seq/session_id/revision/kind/time` with closed kind `changed|deleted` per ADR-0005, cursor/gap→rehydrate, InvalidParams validation of explicit undefined cursor and invalid kind before response/notification, duplicate idempotent via cursor/seq); additive protocol module `packages/opencode/src/private-worker/observation.ts` ↔ `packages/kilo-vscode/src/private-worker/observation.ts` (injected controller, no DB/migration/timer/queue/unsubscribe/production wiring); subscribe is foundation cursor-establishment hook; tests `packages/kilo-vscode/tests/unit/observation.test.ts` + `packages/opencode/test/private-worker/observation.test.ts` prove envelope/snapshot/contiguous/gap→rehydrate/ack/subscribe/malformed/EOF plus closed-kind and explicit-undefined-cursor validation; storage-backed adapter 2026-08-22: `packages/opencode/src/private-worker/changefeed-adapter.ts` ↔ `packages/kilo-vscode/src/private-worker/changefeed-adapter.ts` bridges canonical `Changefeed` Effects (`currentCursor`/`readAfter`/`ack`/`getState`, payload-free, bounded eviction, ADR-0005) to Promise `ObservationDeps` without second store, wired through private-worker `startWorker({ observationDeps })` routing `observation/*` (preserving `initialize`/`ping`/`echo`/`error`, JSON-RPC error codes, lifecycle cleanup) and host `onNotification` forwarding for `observation/changed`; tests `packages/opencode/test/private-worker/observation-changefeed.test.ts` + `packages/kilo-vscode/tests/unit/observation-changefeed.test.ts` prove real DB snapshot/read/ack/eviction-gap/InvalidParams/ack-ahead→InvalidParams and payload-free `deleted` over real `JsonRpcPeer`; full R9 remains Open pending private-worker lifecycle convergence (transport reconnect/worker restart/panel-close/reopen/reload/session-switch observable snapshot/revision convergence) — storage integration proven, lifecycle convergence unproven; five-boundary convergence remains unproven; R11-R14 remain Open; no new store, no selector/readiness change, no HTTP/SSE/SDK removal, no global rebuild/convergence; standalone lease bootstrap 2026-08-22 (corrected 2026-08-22; packaging corrigendum 2026-08-22): additive gated (KILO_PRIVATE_WORKER_STANDALONE=1 + absolute KILO_DB) file-backed canonical DB via leased Database.layerFromPath (lease/marker/identity) + createChangefeedDeps, isolated from production HTTP/SSE, lifetime cleanup on worker dispose/EOF/SIGINT/SIGTERM (process 'exit' not awaited; stale-PID recovery via Database.layerFromPath on next acquire), idempotent cleanup with signal/stdin/interval listener removal, real-child proof (initialize + observation/snapshot/read/ack/subscribe over real child, empty cursor, direct rehydrate:true on cursor-ahead, InvalidParams on truly invalid cursor, lease held while alive, released on close/EOF, reacquired sequentially; lease exclusivity is provided by Database.layerFromPath but concurrent-owner behavior was not exercised in this increment); packaging: default CJS `dist/private-worker/worker.js` lightweight (no `Effect`/`Database`/`bun:sqlite`, 18 KiB) and standalone ESM `dist/private-worker/standalone-worker.mjs` (`conditions: ["node","import"]`, `node:sqlite`, bundled graph, no `bun:sqlite`/TLA, 927 KiB), `packages/core/src/cutover/archive-path.ts` pure helper, host selects ESM only when gate + absolute `KILO_DB`, `bun run compile` passes — full R9 still Open pending five-boundary lifecycle convergence (transport reconnect/worker restart/panel-close/reopen/reload/session-switch snapshot/revision convergence); R11-R14 remain Open |
