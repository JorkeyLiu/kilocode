...
- Agent manifests and session tool toggles may narrow permissions but never widen
  enclosing restrictions. An explicit user approval with bounded scope/lifetime may resolve ordinary `ask` and class (b)/(c) `ask-ceiling` under R18 when it is a single exact match for the same operation (same session, same agent identity, same permission ID, exact canonical pattern set — `once` correlated via `permission:<permissionRequestId>`); that single exact approval resolves every matching ordinary `ask` and every class (b)/(c) `ask-ceiling` for that exact request (no separate class discriminator). No approval resolves class (a) absolute runtime hard deny, any other `deny`, any wildcard/broad ceiling attempt, permission `*`, or `allow-everything` beyond ordinary `ask` via `allow-everything` only. Class (a) is an absolute deny and is never overridden by policy, approval, or allow-everything.
...
- The free-form question flow (a runtime ask rendered to the user) uses permission ID `question`, while the question tool invocation (a harness tool a session invokes to ask the user) uses permission ID `question_tool`. Both are distinct permission strings with separate registry entries and separate provenance identities (section 3.2), removing the current enforcement ambiguity between `question` and legacy `mcp` rules.
...
- Explicit approvals are runtime-owned per-session operation records with bounded
  ...
  ask. Bounded approvals: a single exact `once`/`session` approval for the exact operation (same session, same agent identity, same permission ID, exact canonical pattern set — `once` via `permission:<permissionRequestId>`) resolves every matching ordinary `ask` and every class (b) protected-path or class (c) `*.env` `ask-ceiling` for that exact request (no separate class discriminator); thus a mixed ordinary+ceiling request with one matching exact approval yields `allow`. No approval resolves class (a) absolute hard deny, any `deny`, any wildcard/broad pattern, permission `*`, or session allow. "Allow everything" (`permission: "*": "allow"` or session allow-everything) resolves only ordinary policy `ask`; it never resolves any `deny`, any `ask-ceiling` for (b)/(c), or class (a).
- No-rule default is ask. An applicable authored/selected policy document with no matching rule contributes `ask` (no-rule ask); a layer with no authored document/selected manifest/session restriction is non-applicable and contributes no decision; the runtime-ceiling layer is always evaluated and yields an explicit layer decision `no-ceiling` when no ceiling (a)/(b)/(c) matches (provenance `layers[].decision = "no-ceiling"`, not `ask`, not a final `decisive.result`); `no-ceiling` is neutral and does not block `allow`. Provenance layer-decision representation is `deny|ask|ask-ceiling|allow|no-ceiling` for layer contributions; final decisive result is only `deny|ask|ask-ceiling|allow` (never `no-ceiling`). A session tool toggle-on changes availability only (it creates no permission grant); toggle-off adds a session restriction.
...
#### 7.2.1 B9 session-cancellation contract lock（仅调查锁定，未实现）

B9 为 investigation-only 合同锁定：只锁定合同与证据，不实现、不关门、不新增 bounded decision、不改变 R11-R14。具体而言：不实现 `session/abort`、不新增 fd capability、不修改 `handleSyncSession`、不做 cutover；不关闭 Gate B/C/D；本锁定仅覆盖 session cancellation，不扩大到 prompt/revert/delete。本小节是 prospective 调查记录，不是 bounded implementation decision，不构成 R11-R14 之外的已实现语义。

B9 是 prospective cancellation control envelope：它聚合/引用既有 R11/R12 generation operation/outcome facts（§7.2/§9 已有语义），不新增 generation operation kind，不修改 R11-R14 的记录位置、schema 或 retention 归属。后续 `session/abort` 实现若落地，其 outcome 才需要关联这些既有 generation/outcome 事实；当前阶段只做合同与证据锁定。

现行实现差距（证据侧，不构成目标承诺）：当前 abort 路径为 SDK HTTP → `promptSvc.cancel` → `cancelTree`，影响 session 及 descendants 的 queue/intake/background/runner/tool/stream/status；当前返回为 volatile `200 true`，无 operation identity、无 durable outcome、无 scope 校验；VS Code private abort carrier 尚不存在；busy abort 的完整 SSE/持久化顺序未被当前树实证（未证实，不得作为事实引用）。

目标合同（未实现，后续实现的前提）：operation identity 标识一次 session cancellation request，而不是单个 generation；每个 abort request 绑定 canonical directory + session + 该次 cancellation-request identity；权威 outcome 聚合该请求影响的 active generation、queued work、descendant cancellation 与事件发布结果，不能只比较 HTTP status。

| 合同要点 | 目标语义（未实现） |
|---|---|
| SDK authority | SDK HTTP 仍是唯一 authority；private path 不得与 SDK 并行执行第二次 abort |
| No replay | Private observer 不自动 replay abort；SDK 完成 abort 后 private 只做 detached parity observation，不得 replay 或第二次执行 |
| Request scope | Abort request 绑定 canonical directory + session + cancellation-request identity；scope 校验失败不扩散 |
| Outcome aggregation | Outcome 必须关联 active generation、queued work、descendant cancellation、事件发布结果，不能只比较 HTTP status |
| Transport unknown | SDK timeout、epoch drift、descriptor closure、private transport unknown 不改变客户端 stream/session state |
| Tree coverage | 目标覆盖 session + descendants 的 queue/intake/background/runner/tool/stream/status；未证实的 busy 顺序不承诺 |
| Diagnostics | Diagnostics 不输出 session ID、operation ID、prompt 内容、tool 参数或原始错误 |
...
B9-P2.1 dynamic oracle delta（2026-09-04，docs-only evidence limit 补充，未实现，不改上表合同，不关 Gate）：scope 仅为 in-process handler/runtime/persistence/Bus 行为证明，不是 TCP 或 Gate C/D 证据。Ephemeral 外部 in-process probe（真实 AppLayer/typed HTTP routes/GlobalBus/SSE，`:memory:`，test provider，permission allow；临时文件已删除，无 repo artifact）`bun test <temp probe> --timeout 90000` 1 pass 0 fail，记录 timeline 摘要：busy `shell sleep 30` → second shell 409、`prompt_async` 204、messages 0→3、LLM calls 0→0；abort `200 true` 记为 receipt（非取消成功），shell 约 552ms completed（含 `User aborted the command`），queue internal state 清空，status map `{}`/absent，SSE `turn.open`/`turn.close`/`session.status idle`/`session.idle`/shell end；missing/idle/completed re-abort `200 true` 同样记为 receipt。Repo test corroboration (in-process execution；非 TCP、非 cancelTree/HTTP full-tree、非 Gate C/D 证据)：`packages/opencode/test/tool/task.test.ts` direct `runState.cancel` descendant pass（1 pass 26 filtered，非 cancelTree/HTTP full-tree）；`packages/opencode/test/kilocode/server/listener-runtime.test.ts` 保持 unresolved/harness-or-product unknown（HTTP 200 通过但 parent runner 未 interrupt，约 7474ms 超时，不归因产品 bug）。Environmental blocker：workspace TCP liveness 不稳定/blocked（可连接零 response bytes，部分 `us_socket_t kind=invalid` SIGTRAP；prod 对照快速 401），不写成 PASS，真实 HTTP abort timeline 未取得。保留 dynamic unknown：TCP busy 顺序、SDK timeout/late SSE、descendant HTTP failure cause、followup live cancel、新 intake race、partial aggregation；queued slot identity/descendant HTTP outcome 不写成已证明。R11-R14 不变。

B9-P2.1 TCP oracle delta（2026-09-04，plain TCP observed，docs-only，未实现，不改上表合同）：provenance 为 current workspace HEAD `f92690fe6b`，Bun 1.3.14/macOS，isolated XDG/temp DB，plain env cleared（本报告摘要，无原始日志 artifact）。Knowns（分层记录）：HTTP receipt（`/global/config` 200 ~49ms、`/event` 200 SSE + `server.connected`、create 200 ~95ms、abort 200 true ~143ms 仍为 receipt）；runner busy/interrupt（corrected `agent:code` + test model + `sleep 30` busy、second shell 409 SessionBusyError、abort 后 blocked shell 200 ~50ms resolved）；shell response（completed bash part + `<metadata>User aborted the command</metadata>`、post-abort echo 200 reusable）；persistence（messages persisted、final `{}` by design）；SSE（message/part updates → `session.status idle` + `session.idle`；missing/idle re-abort 200 true 仍 receipt）。Probe correction：invalid `agent:build` 首探 500 UnknownError 为 malformed probe input，不记产品故障。Evidence limits：env-only/bogus-fd（`KILO_CLIENT=vscode`/`KILO_PARENT_PID`，fake parent watchdog expected、real parent alive 复现 zero-byte/reset + child 94–98% CPU、non-production supervisor fds gate true）只记 environment/probe limitation，不记真实 VS Code peer composition 证据；exact JS spin path unknown；真实 Extension Host fd3/fd4 remains unknown。In-process timeline、repo corroboration、TCP unstable/blocked 历史边界保留；`accepted-unresolved`/`transport-unknown`/`ambiguous` 语义不变。Gate B 仍 named-open，G3 Active；Gate C/D 不关闭；不新增 carrier/capability；不修改 abort/cancelTree/SDK；R11-R14 不变；B9 implementation 仍未就绪。

B9-P2.1 Extension Host oracle delta（2026-09-04，real Extension Host observed，docs-only，未实现，不改上表合同）：provenance 为真实 E2E `KILO_E2E_SCENARIO=real-lifecycle` Node launcher，owned scratch/evidence（本报告摘要，无原始日志 artifact；外部 temp evidence 非 repo test artifact）。Knowns（分层记录）：ancestry（ExtHost pid 99779 / backend pid 190 / PPID 99779 / port 62964 / epoch 1）；backend fds（`lsof -a -p 190 -d 3,4 -n -P` fd3/fd4 unix endpoints kernel 侧已捕获）；private peer（`privatePeerStatus` available/open，protocol `kilo-private/1.0`，8 capabilities，`hasSessionUpdate` true；PrivatePeer negotiated pid 190 epoch 1）；HTTP/SSE（SSE opened / server connected）；lifecycle（`real-lifecycle` passed）。Evidence limits：Host 对端 fd number 未捕获，只证明到 unix endpoint/pid/epoch 层，不声称双端 fd mapping 完整。Raw handoff malformed sole cause 为 validator drift（`e2e-evidence` 2-cap allowlist 拒绝 `session/fork`；`packages/kilo-vscode/script/e2e-evidence.ts` + `packages/kilo-vscode/tests/unit/e2e-evidence.test.ts` 另行修复），不是 peer 失败，该修复不构成 Gate closure。Read-path 37 divergences（Get 13 / Messages 9 / Children 15）无 per-call failure code/status/opId，保持 unknown，不计 failure rate，不归因产品；session IDs 无 colon、pid/epoch/directory stable、title parity clean 记为观察事实。本次 Host run 未执行 abort；此前 plain TCP/in-process observations 中的 200 true 仅为 receipt，不是 cancellation proof，也不是本次 Host run 的 abort observation。Private observer parity 与 SDK authority 不重叠：private 只做 detached parity observation，SDK 仍是唯一 abort authority。Gate B 仍 named-open，G3 Active；Gate C/D 不关闭；不新增 abort carrier/capability；R11-R14 不变。

B9-P2.1 Host abort oracle delta（2026-09-04，native CLI override，本次文档更新 docs-only，未实现，不改上表合同，不关 Gate）：provenance 为 `KILO_P0_BACKEND_CLI` native override only、非默认 source-wrapper production path proof——`KILO_P0_BACKEND_CLI=<repo>/packages/opencode/dist/@kilocode/cli-darwin-arm64/bin/kilo KILO_E2E_SCENARIO=real-session ... node script/e2e-probe-launch.mjs` exit 0 / `validated true`（本报告摘要，无原始日志 artifact；外部 temp evidence 非 repo test artifact；本次文档更新本身未改代码/测试；primary evidence 见 `evidence/p4.4-g3-b9-session-abort-contract-investigation.md` Host abort oracle delta，本节只做 cross-reference）。Knowns（分层记录）：startup（backend pid 17930 / private peer pid 17930 epoch 1 / port 51055 / hang server 51004 / SSE connected）；A/B abort timeline（A/B busy → Stop A → A idle / B busy → Stop B → both idle；real UI Stop 为唯一 abort path）；backend abort log（Stop A 后 `service=session.processor ... error=Aborted process`，记为 backend 侧信号，非 HTTP receipt）；snapshots/DOM/LLM（real-snap1..8 / DOM evidence / LLM 4 records 全 `e2e-local`/`e2e-model` `violations[]`，只记 matrix，不记 SDK abort call count）；lifecycle（`real-session lifecycle passed` 只记 lifecycle，不当 B9 完整合同闭合；canonical archive stable；`validated true`）。No abort receipt raw body captured，本节不引用 `200 true` body；若出现只记 receipt。Evidence limits：default source-wrapper run 仍为独立 limitation（pid 81735 no port in 30s startup timeout），owned direct source startup matrix 在 Host 外成功不记 wrapper 健康；exact SSE event order beyond captured facts unknown。Authority split：SDK 仍是唯一 abort authority，private 只做 detached parity observation。保留 unknown：queued / partial / timeout / late-SSE / operation aggregation（generation/queued/descendant/event-publication 聚合未证明）。Prior Host composition 边界保留（backend fd3/fd4 backend-only、validator drift fixed separately、read 37 unknown、前次 Host run no abort）与 in-process/TCP deltas 保留。Gate B 仍 named-open，G3 Active；Gate C/D 不关闭；不新增 carrier/capability；不修改 abort/cancelTree/SDK；R11-R14 不变；B9 implementation 仍未就绪。

B9-P2.1 Default source-wrapper real-session Host abort oracle delta（2026-09-04，本次文档更新 docs-only，未实现，不改上表合同，不关 Gate）：provenance 为默认 source-wrapper path、与前次 native override 独立区分——HEAD `0343a02b1b8d9814dd40766b316d21916ac69de4`、`KILO_P0_BACKEND_CLI` absent、默认 `packages/kilo-vscode/bin/kilo` 经 `KILO_E2E_SCENARIO=real-session ... node script/e2e-probe-launch.mjs`、34986ms / `validated true` / `status complete`（本报告摘要；外部 `/tmp/kilo-p0-default-realsession-20260904-155200/` 非 repo artifact，不提交；本次文档更新本身未改代码/测试；primary evidence 见 `evidence/p4.4-g3-b9-session-abort-contract-investigation.md` Default source-wrapper delta，本节只做 cross-reference）。Knowns（分层记录）：startup/peer（backend pid 83832 / port 53573 / PrivatePeer pid 83832 epoch 1 / SSE stream opened；UI Stop 收敛、SDK abort receipt 未独立观察、backend 信号、SSE 顺序、durable outcome 分层，不混淆）；A/B abort timeline（A/B busy → Stop A → A idle / B busy → Stop B → both idle；real UI Stop 为唯一 abort path）；backend abort signal（`session.prompt ... cancel` + `session.processor ... error=Aborted process` 均出现，但首个 Stop A 紧随 B-session Aborted 行，attribution 保持 ambiguous，只记 backend 侧信号，不记 HTTP receipt）；snapshots/persistence（real-snap-1..7 / DOM / closed-A persists / B rehydrate + persist / `real-session lifecycle passed` 只记 lifecycle；canonical archive stable）；LLM（post-abort/final 各 4 generation requests，全 `e2e-local`/`e2e-model` `violations[]`，只记 matrix，不记 SDK abort call count）；parity divergence（`sdk=succeeded priv=failed` + `InstanceRef not provided` observed，不归因 root cause）；teardown（no owned VS Code process、CDP 53550 released、scratch removed、exit 0）。Explicit non-claims：exact SDK `session.abort` body、abort call count、exact SSE order、queued/partial/timeout/late-SSE/aggregation 均 unobserved；不声称 wrapper 健康、production-wide proof、aggregate outcome、Gate closure、readiness。Prior native delta（pid 17930 / port 51055 / hang 51004）保持独立；旧 source-wrapper 30s timeout limitation 仍保留为历史 evidence；本次运行仅增加一次默认-path 成功观察，不证明 wrapper 范围健康或可重复性。Gate B 仍 named-open，G3 Active；Gate C/D 不关闭；不新增 carrier/capability；不修改 abort/cancelTree/SDK；R11-R14 不变；B9 implementation 仍未就绪。

B9-P2.1 Default-path live A/B abort evidence delta（2026-09-04，本次文档更新 docs-only，未实现，不改上表合同，不关 Gate）：provenance 为默认-path live run、与前次 native override 及前次 default source-wrapper 均独立区分——HEAD `88eba1812186876969ad635f57e78553da35e48f`、`env -u KILO_P0_BACKEND_CLI -u KILO_E2E_PROVIDER_BASE_URL -u KILO_DB -u KILO_P0_PERF KILO_E2E_SCENARIO=real-session KILO_E2E_EVIDENCE_DIR=/tmp/kilo-p0-abort-20260904-185800-evidence node script/e2e-probe-launch.mjs`（`packages/kilo-vscode` 下运行）、fixture `e2e-probe-a1f137a6`、43700ms / `validated true` / launcher 57s（本报告摘要；外部 `/tmp/kilo-p0-abort-20260904-185800-evidence/` 非 repo artifact，不提交；本次文档更新本身未改代码/测试；primary evidence 见 `evidence/p4.4-g3-b9-session-abort-contract-investigation.md` Default-path live delta，本节只做 cross-reference）。Knowns（分层记录）：startup/peer（backend pid 4971 / port 63829 / PrivatePeer pid 4971 epoch 1 / SSE connected；SDK receipt、backend 信号、SSE 顺序、durable outcome 分层，不混淆）；sessions（A `ses_f93ee292bffeyQs0GAf47wCG5q` / B `ses_f93ee1f84ffeNiNINPmH1SNl8f`）；abort receipts（`abort-attempts.json` total 4，two records per session，attempts 1/2，全 ok true / 200 / data true，A 94/105ms，B 82/91ms，只记 receipt；两条 directory 记录只记 observed fan-out，不记 retry）；SSE timelines（A/B 各 10 entries seq 0-9，无 drops/truncation）；backend signal（prompt cancel + `Aborted process` + bus turn.close；首个 Stop A 紧随 B-session 行，attribution 保持 ambiguous）；convergence（A idle / B busy → both idle；real UI Stop 为唯一 abort path）；persistence（closed-A persists / B rehydrate + persist / canonical archive stable / lifecycle passed 只记 lifecycle）；LLM（post-abort/final 4 generation requests，全 `e2e-local`/`e2e-model` `violations[]`，只记 matrix）；teardown（clean）。Explicit non-claims：logical SDK abort call count、cross-layer HTTP→backend→SSE order、durable aggregate outcome、queued/partial/timeout/late-SSE/aggregation 均 unobserved；不声称 wrapper 健康、production-wide proof、readiness；parity/InstanceRef noise 不归因 root cause。Prior native（pid 17930）与 prior default（pid 83832）deltas 保持独立；旧 source-wrapper 30s timeout 仍保留为历史 evidence。Gate B 仍 named-open，G3 Active；Gate C/D 不关闭；不新增 carrier/capability；不修改 abort/cancelTree/SDK；R11-R14 不变；B9 implementation 仍未就绪。
...
layout and exact persistence assignment) is resolved 2026-08-18 — required
...
rows). R18 (permission-evaluator bounded semantics) is resolved 2026-08-24 as
specification-only — required by P4, not a P1-P3 blocker, and does not claim
implementation or close the permission-evaluator implementation/test gate (see
row). The tracker (section 9) is the mutable
...
| R9 Observation/hydration implementation details (snapshot/event handshake; revision scope/ordering/idempotency; ephemeral-fact retention) | Bounded implementation decision; the normative contract (section 7.1) fixes the one-owner and lifecycle-convergence constraints but not the wire schema, event sourcing, polling, timer subsystems, or retention. R9 may use a bounded derived changefeed/outbox for reconnect deltas; it is not authoritative history, is not required for reconstruction, and is eligible for automatic truncation after authoritative hydration state exists (ADR-0005; storage spec section 5.2) | P4.2 (P4.2a storage sub-boundary first, then the P4.2b wire/schema freeze; the private-worker observation surface must not ship without it). Required by P4.2 only; not a P1-P3 blocker | Resolved 2026-08-23 — bounded decision proven by real Extension Host five-boundary production observation convergence (panel, reload, switch, reconnect, restart) via private Database.layerNoLease derived observer (resolveCanonicalDbPath/resolveCanonicalDataDir, KILO_DISABLE_CHANNEL_DB=1, absolute gate, no second lease; legacy kilo serve sole lease owner (canonical no-lease observer / legacy lease-owner invariant)), PrivateWorkerHost/PrivateObservationService enabled:true, Memento/InMemory cursor persistence ack-only, singleflight debounced lifecycle triggers ~150ms (panel/window/config/session/peer-closed) via PrivateObservationLifecycleTriggers, reconnect bounded exact-PID shutdown(2000)+doInitialize, no polling. Evidence secret-safe durable `specs/vscode-orchestrator/evidence/p0-baseline/2026-08-23T13-59-07Z-r9-observation/` (manifest.redacted.json + provenance.json, source manifest d2b3b8ecc9572ea4010585a64bacda1e06fdb21a6c7cd8ec7f647de4541e3f26, schema kilo-e2e-evidence/1, scenario r9-observation, success:true, validated:true, status:complete, 25 files, run.log omitted per minimal evidence policy, secret scan no fixture-key) from audited source `/var/folders/9v/733tfwtj2jvgwygmtrbwj9gm0000gn/T/kilo/r9-evidence-1755961000-2082` (audit PASS 0 blocker/acceptable-risk, independent). Evidence shows panel 1→1 continuity true rehydrate false, reload 1→1, switch 1→1 with switchConfirmation e2e-probe-d3cbd796-B, reconnect 1→2 with observation/changed seq2, restart 2→5 rehydrate:true seq3-5 evict cap, r9-dom-evidence persistedCursor 3 final pid 83485 selected true, canonical-gate zeroState 0 archive before/after identical 9f86…, cstate materializationReady true, plan consistent, protocol envelope v1.0 payload-free seq/session_id/revision/kind/time gap→rehydrate; no polling/unbounded queue/new store; HTTP/SSE/SDK bridge preserved per LOCK-009 until P4.4/P4.5; P4.2b lifecycle gate Complete while P4/P4.2 remain Active (P4.3/P4.4/P4.5 pending) |
| R10 Canonical schema/field-registry layout and exact persistence assignment | The normative rules are fixed by this spec — legal source taxonomy (section 3.1), field-registry content (section 3.2), typed composition/materialization/provenance (section 5.1), agent-manifest role (section 5.2), permission composition (section 5.3), and the bidirectional file-editing/WYSIWYG contract (section 5.4). File/asset authority is fixed by R2 (revised 2026-08-13), and R10 is bounded within that topology: exact canonical filenames/layout, the registry entry per remaining field class, legal scope/operator per field, and watcher owner/stamping/conflict implementation details (section 5.4). It does not reopen file authority, the two-level authored scope set, the SecretStorage exception, or the no-migration decision. The persisted-index layout and reconciliation behavior are additionally bounded by section 6 criterion 3: reconciliation must preserve the last-known valid derived index or an explicit invalid/stale presentation and never reintroduce a worker-readiness dependency | P4.1 | Resolved (2026-08-18): the closed 11-field JSONC registry (`packages/kilo-vscode/src/config/registry.ts`) covers the exact `CanonicalConfigPayload` field classes (model, model_variant, model_variant_overrides, subagent_model, subagent_variant, subagent_variant_overrides, default_agent, provider, mcp, permission, instructions). Typed asset classes (agent, command, skill, tool, plugin, rules) exist as typed markdown files in their canonical directories (`packages/kilo-vscode/src/config/types.ts`, `ASSET_DIRECTORIES`) — they are separately registered canonical asset classes, never top-level JSONC fields. Global root: `~/.config/kilo/kilo.jsonc` resolved from platform home via `Roots` class (`packages/kilo-vscode/src/config/paths.ts`, injectable for tests). Project root: `<workspaceRoot>/.kilo/kilo.jsonc`. Canonical project scope exists only when a first VS Code workspace folder is open; with no workspace, the service operates global-only and rejects project-scope writes and asset operations. Each registry entry carries complete scope, operator, owner, persistence, secret handling, snapshot inclusion, provenance, and removal disposition. Validation is enforced by the separate closed `fieldSchemas` mapping and contextual validators (`packages/kilo-vscode/src/config/validate.ts`), not as per-entry registry metadata. SecretStorage credentials use opaque refs only (`secret:kilo.credentials.<scope>.<kind>.<id>`); plaintext never in files. Evidence: `packages/kilo-vscode/src/config/registry.ts` (closed 11-field set `CLOSED_JSONC_FIELDS`), `packages/kilo-vscode/src/config/types.ts` (`ASSET_DIRECTORIES`, `CanonicalConfigPayload`, `CanonicalProviderPayload`, `CanonicalMcpPayload`, `RegistryEntry`), `packages/kilo-vscode/src/config/paths.ts` (`Roots`, `resolveCanonicalPaths`), `packages/kilo-vscode/src/config/validate.ts` (`fieldSchemas`), `packages/kilo-vscode/src/config/secret-adapter.ts` (owned-ref parser) |
| R11 Operation/outcome identity and record location/retention | Bounded implementation decision under the normative Failure/Outcome/Recovery target (section 7.2) and the observation contract (section 7.1): what an accepted semantic operation's identity is, where the canonical Failure/Outcome records live, and their minimal retention under existing storage — no new store mandate. Generation-path operations only (prompt/generation, provider attempt, tool call, permission/question wait, child/background task); config commit outcomes are owned by sections 5/5.4 and are not R11 operations. Durable Failure/Outcome fields persist in the canonical aggregate storage / registered artifact model; diagnostic and panel projections are derived and never create competing stores (ADR-0005; storage spec sections 5.1, 5.4). It does not reopen runtime sole authority for operational facts or the client replay prohibition | P4.2 (P4.2a storage sub-boundary first). Not a P0-P1 blocker | Resolved 2026-08-23 — additive canonical DB foundation `session_operation` (`session_id REFERENCES session(id) ON DELETE CASCADE`, `CHECK` for `op_kind`/`outcome`, indexes, `revision` inspectable) via `packages/core/src/database/migration/20260824000000_add_operation_record.ts` + `packages/core/src/session/sql.ts` + `packages/core/src/session/operation.ts` (consumes R12 `normalize`/nine-field `FailureRecord`/`FIELD_TIERS`/`select` unchanged, persists `select(record,"persist")` full redacted record, stable constructors `prompt:msgId` / `provider:assistantId:attempt` / `tool:assistantId:callId` / `permission:requestId` / `task:childSessionId(:parentCallId)?`, validates `opId` kind prefix/segments and `opId`/`opKind` correspondence, one `BEGIN IMMEDIATE` transaction atomically advances `SessionRevision` with one payload-free `changed` feed row (existing `changed|deleted` kinds/caps unchanged), deterministic `get`/`list` by `op_id`, narrowest transition only `in-flight`→terminal idempotent or forward, rejects cross-kind/cross-identity/terminal regression/terminal→terminal non-identical/`in-flight`→`in-flight` non-identical without revision/feed, retention via 8/6 GiB family cascade (no separate policy), not a file artifact/new DB/retry ledger; tests `packages/core/test/operation-r11.test.ts` (13 pass: migration, five-kind identity, redacted round trip, atomic row+revision+changed feed, idempotent and conflict no double revision, terminal cannot regress, payload-free feed, family cascade/retention) + existing `changefeed-s4`/`retention-s2`/`session-revision` still pass; R13/R14/full R9 remain Open (no recovery/crash/full wiring) |
| R12 Minimum private-runtime Failure/Outcome schema + panel projection/redaction | Bounded implementation decision under section 7.2: the minimum Failure/Outcome schema, the runtime-owned normalization boundary, minimal field tiers (durable vs diagnostic vs panel-visible), runtime-side redaction before persistence/projection, cancellation provenance (at least user stop/steering/timeout/network disconnect/unknown), and a versioned private panel envelope/projection. No complete taxonomy and no byte/depth sanitizer contract freeze. The private error envelope's version/compatibility is owned by R1/R9/R12 as appropriate | P4.2. Not a P0-P1 blocker | Resolved 2026-08-23 — additive pure private-runtime module packages/opencode/src/private-worker/failure.ts ↔ packages/kilo-vscode/src/private-worker/failure.ts (semantic parity): versioned 1.0 Failure/Outcome envelope; sole runtime-owned normalization boundary normalize (provider/session/tool/permission/worker/transport → FailureRecord); classification classify separate from recovery (no timers/retry/scheduling in module); minimal 9-field schema without taxonomy freeze (opId/opKind/outcome/code/message/time/cancel/detail/stack; small closed code set with stable unknown fallback, never invented codes); field tiers as exposure ceilings (durable < diagnostic < panel-visible) with single FIELD_TIERS source driving persist/diagnose/project selection (project emits panel-visible only); runtime-side redaction inside normalize before any persistence/projection emission (key-based secret scrub + string value scrub, cycle guard, depth cap, message 500/detail 1000/stack 2000 caps); cancellation provenance closed set of exactly five sources (user_stop/steering/timeout/network_disconnect/unknown) → outcome abandoned; versioned private panel envelope buildPanelEnvelope with runtime-side projection (client never re-derives failure); no production wiring, no new store/DB table, no HTTP/SSE/SDK/session/retry change (LOCK-009 bridge remains); record location/retention stays R11, recovery accounting R13, worker-crash disposition R14; tests packages/opencode/test/private-worker/failure.test.ts + packages/kilo-vscode/tests/unit/failure.test.ts (39+39 tests, 146+146 assertions, incl. stack-secret redaction); audited 2026-08-23 ACCEPT, 0 blockers |
| R13 Recovery accounting/coordination and low-level retry visibility | Bounded implementation decision under section 7.2 — P4.2 is accounting/coordination only: owner/scope, budget consumed/termination, next-at occurrence time, provenance, and visibility of nested low-level attempts within the owning operation. It may reuse current bounded behavior. Retryability algorithms, delay shapes, and future recovery features are post-foundation (section 7.3). It does not preserve old retry shapes, delay ladders, the environment retry flag, or the auto-continue implementation | P4.2. Not a P0-P1 blocker | Resolved 2026-08-23 as implementation decision — additive pure private-runtime module `packages/opencode/src/private-worker/recovery.ts` ↔ `packages/kilo-vscode/src/private-worker/recovery.ts` (semantic parity): versioned 1.0 recovery accounting; closed owner/scope/provenance/layer/termination sets (`runtime`/`operation`/`runtime,user,offline-restore`/`semantic,sdk,provider,transport,task`/`budget-exhausted,non-retryable,replay-unsafe,cancelled,completed`); immutable `RecoveryRecord`/`RecoveryAttempt` validation with R11 opId/opKind correspondence (five identity forms: `prompt:msgId`/`provider:assistantId:attempt`/`tool:assistantId:callId`/`permission:requestId`/`task:childSessionId(:parentCallId)?`) locally without importing core; `create`, `charge`, `terminate`, derived `snapshot` (projection only, no second store); `charge` requires unique attempt id for idempotency (exact replay returns unchanged, conflicting facts reject), enforces budget before accepting (consumed exactly once, nested layer visible, shared consumption), optional `nextAt` >= time and cleared on termination, explicit replay safety (`nextAt` requires `replaySafe === true`, unsafe → `terminate` with `replay-unsafe` clears `nextAt`); narrowest `limit=0` handling (no charge accepted, consumed stays 0, `budget-exhausted` termination explicit) and charge reaching limit (last allowed charge succeeds, next charge fails `budget exhausted`, no overrun); runtime occurrence times only (never receipt), no clock/timer, no retry algorithm/delay constants, no DB/migration/store/Map/service, no observation notification, no R11/R12 contract change, no production wiring; tests `packages/opencode/test/private-worker/recovery.test.ts` + `packages/kilo-vscode/tests/unit/recovery.test.ts` (32+32 tests, 202+202 assertions proving closed schema/parity, five op kinds, immutable/idempotent charging, nested low-level visibility and shared consumption, runtime/user/offline provenance, occurrence-vs-nextAt, unsafe replay termination, budget exhaustion/no overrun, explicit terminal reasons, no timer/retry constants/DB imports, derived projection); implementation evidence only (32+32 tests, 202+202 assertions, typecheck, lint, knip, parity), independent re-audit 2026-08-23 ACCEPT, 0 blockers, one evidence-count style correction applied; R14/full R9 remain Open (no worker-crash/full observation wiring) |
| R14 Worker-crash in-flight disposition | Bounded implementation decision under sections 7.2 and 7.1: how an accepted in-flight operation at a worker crash converges to a recorded disposition with resource cleanup and no silent client replay — without requiring resumability and without a new persistent operation ledger | P4.2. Not a P0-P1 blocker | Resolved 2026-08-23 — additive pure private-runtime module `packages/opencode/src/private-worker/disposition.ts` ↔ `packages/kilo-vscode/src/private-worker/disposition.ts` (semantic parity): versioned 1.0 worker-crash disposition; closed outcome `succeeded,failed,ambiguous,in-flight,superseded,abandoned` and provenance `worker-crash`; explicit cleanup fact `released` and occurrence/receipt times (receipt >= occurrence, distinct); `replayed:false` (no silent replay); R11 five-form identity `prompt:msgId`/`provider:assistantId:attempt`/`tool:assistantId:callId`/`permission:requestId`/`task:childSessionId(:parentCallId)?` with local `parseOpId` cross-kind rejection; `create` (in-flight) + `apply` (crash disposition) with `validateRecord` and `isTerminal`; idempotent same crash event replay (exact facts return same), conflicting same-op facts reject, only `in-flight`→terminal allowed, terminal exact replay allowed, cross-kind/cross-identity/regressive reject; immutable frozen records; caller-supplied times/facts, no clock/timer; pure no DB/migration/Map/service/timer/queue/notification/retry/production import or wiring; tests `packages/opencode/test/private-worker/disposition.test.ts` + `packages/kilo-vscode/tests/unit/disposition.test.ts` (25+25 tests) prove five-forms/closed sets/crash provenance/cleanup/occurrence vs receipt/no silent replay/idempotency/conflicting/terminal forward/terminal replay/cross-kind/cross-identity/regressive/static purity/immutability; does not claim R9 or P4.2 complete |
...
| R18 Permission evaluator bounded semantics | Bounded specification decision under section 5.3; normative composition hard-safety ceilings, ownership, identifiers, approval lifetime, provenance, child propagation, single-document wildcard ordering, and layered composition. Must not be read as implementation or gate closure; no runtime code, no new effective-config source, no P4.3 change | P4 (permission-evaluator gate; not a P1-P3 blocker) | Resolved (2026-08-24) as specification-only bounded decision (no runtime implementation claimed; evaluator gate remains open; current flat last-match behavior is non-normative and not claimed satisfied — evidence: `specs/vscode-orchestrator/evidence/p4-permission-evaluator-semantics.md`): 1) Hard-safety ceiling catalog — complete v1 catalog of exactly three ceilings; additions require a new bounded decision; (a) any applicable runtime hard deny is absolute deny — cannot be overridden by policy, approval, or allow-everything; (b) protected canonical control/config paths cap mutating access at ask unless an explicit exact agent/path safety approval exists — protected paths are: global and project config roots (`~/.config/kilo/kilo.jsonc`, `<workspaceRoot>/.kilo/kilo.jsonc`), all typed/config assets under canonical `<workspaceRoot>/.kilo/` (legacy `.kilocode/` is a pre-cutover safety boundary only until the P4.3 reader cutover — not a canonical target and does not survive cutover; its safety protection remains relevant only before cutover) except `plans/` (agent, command, rules, skills, workflows, config assets), and root control files `kilo.json`, `kilo.jsonc`, `opencode.json`, `opencode.jsonc`, `AGENTS.md`; mutating permission IDs covering those paths cap at ask; only an exact canonical agent+path safety approval resolves the ask; broad/wildcard patterns (`*`, `**`, prefix `*`), permission `*`, and allow-everything/session allow do not bypass; (c) broad/wildcard read grants for `*.env` and `*.env.*` cap at ask — only an explicit exact approval for the exact `*.env` pattern/file resolves the ask; literal `*.env.example` is exempt (allow permitted); (a) is absolute deny, (b) and (c) are ask ceilings — a single exact bounded approval for the same operation (same session, same agent, same permission, exact canonical pattern set) resolves every matching ordinary `ask` and every (b)/(c) `ask-ceiling` for that exact request (no separate class discriminator); no approval resolves (a), any `deny`, any wildcard/broad ceiling attempt, permission `*`, or `allow-everything` beyond ordinary `ask` via `allow-everything` only. 2) Evaluator ownership — authoritative enforcement is runtime-owned in the private CLI/runtime (`packages/opencode`); the extension authors canonical policy (global/project `permission` fields, typed agent manifests) and renders decisions; shared permission IDs/types may live in the core package (`packages/core`) but carry no enforcement authority; no extension-side authoritative enforcement, no second authority/store. 3) Permission identifiers — canonical IDs are `question` for the free-form question flow (free-form `question` permission, runtime ask rendered to user) and `question_tool` for the question tool invocation (`question_tool` permission); each has a separate registry entry and distinct request/decision provenance identities; `mcp` is not an authorization alias — discovery/availability (`mcp` keyed records) is separate from authorization; built-in identifiers follow the config contract (for example `read`, `edit`, `bash`, `glob`, `grep`, `list`, `task`, `external_directory`, `skill`, `agent_manager`, `webfetch`, `websearch`, `lsp`, `doom_loop`, `todowrite`, `todoread`, `notebook_read`, `notebook_edit`, `notebook_execute` plus `question`/`question_tool`); no invented arbitrary plugin IDs beyond those represented by the `permission` field contract. 4) Per-session approval scope/lifetime — ephemeral runtime memory scoped to a session and exact request patterns; `once` applies to one operation identity `operationId = permission:<permissionRequestId>` (for example `permission:per_abc123`), correlated via `request.operationId == approval.operationId`, distinct from `request.permission` (`edit`, `question_tool`); `session` applies only to the same session, agent identity, permission ID, and exact canonicalized request pattern set until session end or explicit revoke; no wildcard or broader pattern counts as exact for ceiling-protected paths; child sessions never receive parent approvals; parent-allow inheritance is forbidden; runtime crash/disposal drops all approvals; durable policy changes happen only through explicit canonical file editing, never as a side effect of a permission ask/reply; a single exact `once`/`session` approval for the same operation (same session, same agent, same permission, exact canonical pattern set) resolves every matching ordinary `ask` and every class (b)/(c) `ask-ceiling` for that exact request (no separate class discriminator; does not cover class (a), any deny, wildcard/broad, permission `*`, or allow-everything); exact bounded approvals may resolve only (b)/(c) ask ceilings. 5) Provenance structure — v1 machine-readable in-memory result shape with schema version `1`, request `permissionRequestId`/`operationId` (`operationId = permission:<permissionRequestId>`, for example `permission:per_abc123`) and `request.permission` (`edit`, `question`, `question_tool`) plus canonical patterns, all contributing layers/policies evaluated (provenance layer-decision representation is `deny|ask|ask-ceiling|allow|no-ceiling` for `layers[].decision`; `no-ceiling` valid only for the always-evaluated `runtime-ceiling` when no (a)/(b)/(c) matches — explicit neutral, not `ask`, never a final `decisive.result` which is only `deny|ask|ask-ceiling|allow`; only applicable policy layers contribute otherwise; non-applicable policy layers with no authored document/selected manifest contribute no decision), source kind and canonical path or asset ID per layer (global file, project file, selected agent manifest, session restrictions, runtime safety) plus per-layer decision, rule identity/pattern/action/order, decisive result/reason (`deny`/`ask`/`ask-ceiling`/`allow` only — never `no-ceiling` — plus which ceiling/rule decided, with `decisive.ceilingId` (a)/(b)/(c) when `ask-ceiling`), and approval metadata when present (`once`/`session`, scope, pattern set, expiry, `operationId` for `once`; a single exact approval for the same operation (same session, same agent, same permission, exact canonical pattern set) is recorded as covering both matching ordinary and ceiling asks when applicable); an applicable authored/selected document with no matching rule contributes `layers[].decision = "ask"` (no-rule `ask`) with `decisive.reason` `<layer>-no-rule-ask`; runtime neutral `no-ceiling` is included as `layers[].decision = "no-ceiling"` but does not create `ask`; non-applicable layers contribute no layer entry; provenance is diagnostic/read-model data and is not a second authority/store. 6) Child propagation — children inherit enclosing denies and session restrictions, never parent allows or approvals; each child re-evaluates global, project, its own selected agent manifest, and its own session restrictions; nested descendants repeat the same rule; no approval leakage across sibling or parent-child boundaries. 7) Single-document wildcard/rule ordering — within each owned policy document only: select by exact literal match first, then greater literal specificity (more literal characters and fewer wildcard tokens; `*` and `**` count as wildcard tokens), then later declaration order for a tie; this ordering never crosses policy layers; cross-layer ordering is by ceiling/layer composition only. Missing rules in an applicable authored/selected document contribute `ask` (no-rule ask); non-applicable layers contribute no decision; runtime neutral `no-ceiling` never ordered. 8) Explicit composition algorithm — runtime-ceiling layer is always evaluated and yields `deny` (a), `ask-ceiling` (b/c), or explicit neutral `no-ceiling` (`layers[].decision = "no-ceiling"`, not `ask`); an authored/selected policy layer is applicable, a layer with no authored document/selected manifest/session restriction is non-applicable and contributes no decision; an applicable authored/selected document with no matching rule contributes `ask` (no-rule ask); neutral `no-ceiling` does not block `allow`; provenance layer-decision representation is `deny|ask|ask-ceiling|allow|no-ceiling` for layers, final `decisive.result` is only `deny|ask|ask-ceiling|allow` (never `no-ceiling`); deny dominates ask dominates allow (LOCK-006); evaluation: collect per-layer decisive action (layer `decision`) over runtime-ceiling (always) plus applicable policy layers using single-document ordering; if any layer yields `deny` => `deny` (class (a) never resolved); otherwise if any layer yields `ask`/`ask-ceiling` (including applicable no-rule `ask`) => `ask`/`ask-ceiling` unless every ask is resolved — a single exact `once`/`session` approval for the same operation (same session, same agent, same permission, exact canonical pattern set — `once` via `permission:<permissionRequestId>`) resolves every matching ordinary `ask` and every class (b)/(c) `ask-ceiling` for that exact request (no separate class discriminator; no wildcard, permission `*`, or allow-everything for ceilings); ordinary `ask` may also be resolved by `allow-everything`; every ceiling ask and every ordinary ask must be covered before `allow`; a mixed ordinary+ceiling request with one matching exact approval therefore yields `allow`; if any ordinary `ask` remains unresolved => `ask`; if any ceiling `ask-ceiling` remains unresolved => `ask-ceiling` (deny still dominant); `allow` only when every applicable policy layer has decisive `allow`, runtime is neutral `no-ceiling` (not deny/ask-ceiling), and at least one `allow` exists (e.g., global `allow` + existing project document with no matching rule => project applicable `ask` => overall `ask`; standalone `question_tool` `allow` where other policy layers non-applicable and runtime `no-ceiling` neutral => `allow` if all applicable allow); toggles only change availability or add restrictions; no-rule across all applicable layers defaults to `ask`; cross-layer rules never override one another. No implementation/test gate closure claimed; tracker permission-evaluator gate remains unchecked; P4.3 manual checklist remains untouched and pending.

B9 session-cancellation investigation record — not a bounded decision row, no new R, no R11-R14 change, no Gate B/C/D closure（未实现，未关门）：B9 不新增 R、不修改 R11-R14 的记录位置与语义，只以前提方式引用/聚合既有 R11 operation identity 与 R11-R14 outcome/failure 事实，不新增 generation operation kind；SDK authority 与 private parity-only 语义不变；architecture governance 不变（canonical docs 仅随实现变更，当前运行时边界未改变，只有 prospective contract/evidence 更新）。后续 `session/abort` 实现以前提为准：SDK HTTP 唯一 authority、无 private 第二次执行、无自动 replay、cancellation-request identity 与 outcome aggregation 按 section 7.2.1 合同执行。规范位置以 §7.2.1 调查记录为准，本段仅为可发现性指针。
