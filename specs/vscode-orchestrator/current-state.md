...
- The standalone no-lease private worker exposes a bounded/paginated canonical-DB session-list projection (`observation/list` via `createSessionListDeps` over `SessionTable` ordered `time_updated DESC, id DESC`, `GlobalListCursor` v1 `updated+id` `base64url`, `limit` 1..500, `nextCursor`; fields `id/title/parentID/directory/createdAt/updatedAt`), but Agent Manager has not consumed it and the generated SDK remains the projection authority.
