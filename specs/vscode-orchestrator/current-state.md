...
- Agent Manager session-list paging and bounded single-session detail now consume the private `observation/list` + `observation/get` projections; the remaining unresolved boundary is that messages and session lifecycle still depend on the SDK path, with no private authority beyond paged list + detail and no transport removal.
