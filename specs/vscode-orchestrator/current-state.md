...
- Agent Manager session-list paging, bounded single-session detail, and paged plus full-read (`limit=0`) messages now consume the private `observation/list` + `observation/get` + `observation/messages` projections, and transcript export follows the same full-read boundary; the remaining unresolved boundary is that session lifecycle still depends on the SDK path, with no private authority beyond list + detail + messages and no transport removal.
