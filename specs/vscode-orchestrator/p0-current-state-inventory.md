...
| Preset metadata keys | `kilocode/provider/metadata.ts` |
...
| H-5 | MCP | `packages/opencode/src/mcp/index.ts`, `mcp/auth.ts`; existing tests `test/kilocode/mcp-migrator.test.ts`, `test/kilocode/mcp-oauth-callback.test.ts` | Gap entry with references (fixture `:480-493`) | MCP tools in a panel session unproven; baseline uses empty MCP registry by convention |
