...
| Preset providers/catalog/onboarding/org sources (LOCK-006) | P4.4 | `opencode/src/kilocode/provider/provider.ts:42` (`KILO_BUNDLED_PROVIDERS`); `kilocode/provider/models-api.json` (3.0 MB); `kilocode/provider/metadata.ts`; `provider/provider.ts:1383-1392` (models.dev), `provider/models.ts:47-51` (fallback); `kilocode/server/provider-auth-lifecycle.ts`; `kilocode/custom-provider.ts`, `custom-provider-save.ts`, `custom-provider-delete.ts` | `packages/opencode/test/` provider suites (existing) | `packages/kilo-docs/pages/contributing/architecture/cli-runtime.md` ("Outbound provider authentication", "Provider routing") | v2 SDK `Provider` (2), `Oauth` (2), `Auth` (5), `Organization` (1), `Kilo.profile`/`Kilo.auth-status` | Auth records (`api`/`oauth`/`wellknown`), org modes (`config.ts:512`), well-known remote config (`config.ts:586-639`), org IDs in model fetch | - | - |
...
| Source | Evidence |
|---|---|
| Preset provider identities (bundled loaders) | `kilocode/provider/provider.ts:42` (`KILO_BUNDLED_PROVIDERS`, today only `@kilocode/kilo-gateway`) |
| Checked-in catalog (models-dev snapshot) | `kilocode/provider/models-api.json` (3.0 MB) |
| Preset metadata keys | `kilocode/provider/metadata.ts` (deleted P4.4-T5 — preset `providerMetadata` helper removed; optional `Provider.Info` `metadata` field remains optional per LOCK-009) |
...
| R-5 | 3.0 MB checked-in models catalog + preset loaders | `kilocode/provider/models-api.json`; `kilocode/provider/provider.ts:42`; `provider/models.ts:47-51` | Config/provider init | LOCK-006 removal (P4.4) |
