...
> **Amendment (2026-09-01, P4.4-G2 preset catalog and models.dev fallback removed — LOCK-006).** Bounded P4.4-G2 deletes the preset provider catalog and models.dev fallback machinery: `packages/opencode/src/kilocode/provider/models-api.json` (3 MB), `packages/core/src/models-dev.ts` (disk `models.json` / `models-<hash>.json`, `GET ${source}/api.json` fallback, 5-min TTL, 60-min background refresh, `kilo models --refresh`), `packages/core/src/kilocode/models-refresh.ts`, `packages/core/src/plugin/models-dev.ts`, `packages/opencode/src/provider/models.ts`, `packages/opencode/src/kilocode/provider/models-refresh.ts` / `models-snapshot-shape.ts`, `script/kilocode/refresh-models.ts` / `models-snapshot.ts`, build embedding `KILO_MODELS_DEV` / `MODELS_SNAPSHOT_RELATIVE` / `refresh:models` script and `KILO_MODELS_URL` / `KILO_MODELS_PATH` / `KILO_DISABLE_MODELS_FETCH` controls deleted; `ModelsDev` service/layer wiring removed from `provider/provider.ts`, `effect/app-runtime.ts`, `server/routes/instance/httpapi/server.ts` / `handlers/provider.ts`, `cli/cmd/models.ts` / `providers.ts` / `github.handler.ts`, `test/preload.ts`, and `kilo-vscode` source wrapper (`KILO_MODELS_PATH` injection removed). Generic `BUNDLED_PROVIDERS` adapters, `KILO_MODEL_SCHEMA_EXTENSIONS` / `patchConfigModel`, custom-provider save/delete/auth, and old CLI/TUI/server/generated-SDK surfaces retained per LOCK-006/009; no second runtime/listener added per LOCK-010/011. Evidence: `packages/opencode/test/kilocode/p4-4-g2-preset-catalog-removal.test.ts` (files absent, no production `ModelsDev`/`models-api`/`KILO_MODELS_DEV` reference, seams removed, catalog-deleted while adapters/custom paths preserved) and `kilo-vscode` `source-wrapper` absence test. P4.4 and P4.4-G2 remain Active/residual; no phase/gate closure claimed.

> **Amendment (2026-08-28, P4.4 bounded — reference-only `.opencode` filesystem scanner and synthetic migration notification removed; P4.4 remains Active/residual; no phase/row closure).** Bounded P4.4 removal deletes the remaining reference-only `.opencode` filesystem diagnostic scanner and synthetic migration notification: `packages/opencode/src/kilocode/config/config.ts` `detectOpencodeConfig`/`opencodeConfigNotification`/`OPENCODE_NOTIFICATION_ID`/`CONFIG_DOCS_URL`/`isConfigDir` `flagDir` param and `existsSync` import deleted (`isConfigDir` remains `dir.endsWith(".kilo")` canonical only), `packages/opencode/src/kilocode/server/httpapi/handlers/kilo-gateway.ts` gateway notice append (`KilocodeConfig.opencodeConfigNotification` + `Instance.directory`/`Flag.KILO_DISABLE_PROJECT_CONFIG` synthesis) and now-unused `KilocodeConfig`/`Flag`/`Instance` imports deleted (cloud `fetchKilocodeNotifications` response preserved). No extension/webview consumer or public contract required them. Tests updated to assert absence/no legacy scanner while preserving canonical `isConfigDir`/`KILO_DIR_SUFFIXES` and gateway cloud behavior. Canonical global/project config loading, `KILO_DISABLE_PROJECT_CONFIG` semantics where still used, `Global.Path.config` + `KILO_CONFIG_DIR` override and sandbox deny, `.kilocode` theme compatibility, generated SDK, and P4.3 cutover remain untouched per LOCK-001/002/003/004/009. Does not close P4.4, P4.5, P5, or unrelated source/storage rows.
...
> **Amendment (2026-08-27, P4.4 ModelCache residual removal — ModelCache service boundary deleted, LOCK-MODELCACHE-001).** `packages/opencode/src/provider/model-cache.ts` is physically deleted (boundary deleted, not replaced). `packages/opencode/src/effect/app-runtime.ts` no longer exposes a ModelCache injection seam (`ModelCache`/`ModelCacheLayer` absent), `packages/opencode/src/server/routes/instance/httpapi/server.ts` `AppOptions` no longer exposes it, and callers `packages/opencode/src/kilocode/server/provider-auth-lifecycle.ts`, `packages/opencode/src/kilocode/server/custom-provider-save.ts`, `packages/opencode/src/kilocode/server/custom-provider-delete.ts`, `packages/opencode/src/kilocode/anaconda-desktop/service.ts` no longer reference `ModelCache`. Generic catalog `packages/opencode/src/kilocode/provider/models-api.json` deleted P4.4-G2 (3.0 MB removed, historical catalog-preserved wording superseded 2026-09-01), `packages/core/src/models-dev.ts` deleted P4.4-G2 (disk/network/refresh removed); `packages/opencode/src/provider/provider.ts` `BUNDLED_PROVIDERS` and `KILO_MODEL_SCHEMA_EXTENSIONS`/`patchModelsDevModel` remain per LOCK-006; HTTP/SSE/generated-SDK transport unchanged per LOCK-009. Evidence: `packages/opencode/test/kilocode/p4-4-model-cache-removal.test.ts` (file absent, no production `ModelCache` reference, seams removed, catalog deleted P4.4-G2). P4.4 remains Active/residual; no row/phase closure claimed. Prior T7 (2026-08-26) description of `provider/model-cache.ts` as invalidation-only `clear` is historical and superseded by this deletion.

> **Amendment (2026-08-28, P4.4 residual package — legacy filesystem discovery fallbacks removed + audit correction evaluator/MCP canonical narrowing (P4.4 remains Active/residual; no phase/row closure)).** Bounded P4.4 residual-removal package removes inert legacy filesystem discovery fallbacks from effective-config, project identity, permission, and MCP paths while preserving canonical `.kilo` behavior per LOCK-002/003/004/005. Source: `packages/opencode/src/kilocode/paths.ts:32-76` `globalDirs()` → `~/.kilo` only and `skillDirectories()` → `targets: [".kilo"]` only; `packages/opencode/src/kilocode/project-id.ts:41-47,83-89` `getProjectIdFromConfig()` → `.kilo/config.json` only and `resolveProjectId()` → `.kilo -> git` only; `packages/opencode/src/kilocode/permission/config-paths.ts:9-11,62,93` `CONFIG_DIRS` → `[".kilo/"]` only and `isAbsolute()` → `~/.kilo` only; **audit correction (2026-08-28): `packages/opencode/src/permission/evaluator.ts:208-231` `isGlobalProtectedPath` canonical `Global.Path.config`+`~/.kilo`+`~/.config/kilo` only (no `~/.kilocode`) and lexical `.kilo` only, `isProtectedPath` `rootFiles` `kilo.json[kilo.jsonc], AGENTS.md` only (no `opencode.*`); `packages/opencode/src/cli/cmd/mcp.ts:405-436` `resolveConfigPath` canonical `.kilo/kilo.jsonc|json` (project) and `kilo.jsonc|json` (global) only (no `.kilocode`, no `opencode.*`) exported**. Preserves `theme.tsx:564` `[".kilocode",".kilo"]` per LOCK-004 and `kilocode/config/config.ts` `isConfigDir()` canonical `.kilo` — **update 2026-08-28 bounded P4.4: `detectOpencodeConfig()` reference-only `kilo.local.opencode-config-detected` synthetic notification deleted (no `.opencode` scanner remains; cloud `fetchKilocodeNotifications` remains; does not close P4.4/P4.5/P5)**. Tests: `paths.test.ts`/`project-id.test.ts`/`permission/config-paths.test.ts` rewritten to absence, `p4-4-legacy-filesystem-discovery-removal.test.ts` (static absence + canonical preservation + runtime `false`/`true` + behavioral evaluator `ask-ceiling` vs `ask`, MCP resolver canonical vs legacy, `KILO_CONFIG_DIR` live `Flag`/`Global.make()` override, sandbox `profile` deny) with env restoration, sorted `global-health < legacy < managed`. Docs: matrix row 5 updated with evaluator/MCP narrowing (rows 5,7,12,13 remain residual/open per LOCK-001); runtime §8.1 historical sources already canonical-only. `KILO_CONFIG_DIR -> Global.Path.config` override and sandbox deny preserved per LOCK-005; no provider/transport/storage/convergence/SDK change.

> **Amendment (2026-08-27, P4.4 Open Config legacy diagnostic cleanup).** The VS Code Open Config diagnostic inventory (`packages/kilo-vscode/src/kilo-provider/config-file.ts` → `open-config.ts` → `KiloProvider` → `Settings.tsx`) is now limited to canonical authorities: resolved global `kilo.jsonc` (`KILO_CONFIG_DIR` controls resolved `Global.Path.config` root) and `<workspaceRoot>/.kilo/kilo.jsonc`. Legacy source taxonomy (`sourceXdg`/`home*`/`env*`/`project*`, `KILO_CONFIG`/`KILO_CONFIG_CONTENT`/`.kilocode`/`.opencode`/`project-root`/non-`kilo.jsonc` alternatives, `legacy`/`virtual`/`statusLoadedLegacy` badges) removed from `config-file.ts`, `open-config.ts` `Labels`/status, `webview-messages.ts` `OpenConfigFileRequest.labels`, `Settings.tsx`, and all 20 webview i18n dictionaries (replaced with `settings.config.source.global`/`local`). Canonical creation/opening, localization, `KILO_DISABLE_PROJECT_CONFIG` semantics, and `KILO_CONFIG_DIR`→`Global.Path.config` compatibility + sandbox deny remain. Evidence: `tests/unit/open-config.test.ts` and `tests/unit/p4-4-open-config-legacy-absence.test.ts`. P4.4 remains Active/residual; no canonical loader/server/SDK change.

> **Amendment (2026-08-27, P4.4 ModelCache residual removal — ModelCache service boundary deleted, LOCK-MODELCACHE-001).** `packages/opencode/src/provider/model-cache.ts` is physically deleted (boundary deleted, not replaced). `packages/opencode/src/effect/app-runtime.ts` no longer exposes a ModelCache injection seam (`ModelCache`/`ModelCacheLayer` absent), `packages/opencode/src/server/routes/instance/httpapi/server.ts` `AppOptions` no longer exposes it, and callers `packages/opencode/src/kilocode/server/provider-auth-lifecycle.ts`, `packages/opencode/src/kilocode/server/custom-provider-save.ts`, `packages/opencode/src/kilocode/server/custom-provider-delete.ts`, `packages/opencode/src/kilocode/anaconda-desktop/service.ts` no longer reference `ModelCache`. Generic catalog `packages/opencode/src/kilocode/provider/models-api.json` deleted P4.4-G2 (3.0 MB removed, historical catalog-preserved wording superseded 2026-09-01), `packages/core/src/models-dev.ts` deleted P4.4-G2 (disk/network/refresh removed); `packages/opencode/src/provider/provider.ts` `BUNDLED_PROVIDERS` and `KILO_MODEL_SCHEMA_EXTENSIONS`/`patchModelsDevModel` remain per LOCK-006; HTTP/SSE/generated-SDK transport unchanged per LOCK-009. Evidence: `packages/opencode/test/kilocode/p4-4-model-cache-removal.test.ts` (file absent, no production `ModelCache` reference, seams removed, catalog deleted P4.4-G2). P4.4 remains Active/residual; no row/phase closure claimed. Prior T7 (2026-08-26) description of `provider/model-cache.ts` as invalidation-only `clear` is historical and superseded by this deletion.

> **Amendment (2026-08-28, P4.4 residual package — legacy filesystem discovery fallbacks removed (P4.4 remains Active/residual; no phase/row closure)).** Bounded P4.4 residual-removal package removes inert legacy filesystem discovery fallbacks from effective-config and project identity/permission paths while preserving canonical `.kilo` behavior per LOCK-002/003/004/005. Source: `packages/opencode/src/kilocode/paths.ts:32-76` `globalDirs()` → `~/.kilo` only and `skillDirectories()` → `targets: [".kilo"]` only (no `".kilocode"`); `packages/opencode/src/kilocode/project-id.ts:41-47,83-89` `getProjectIdFromConfig()` → `.kilo/config.json` only (no `.kilocode` loop/fallback) and `resolveProjectId()` → `.kilo -> git` only; `packages/opencode/src/kilocode/permission/config-paths.ts:9-11,62,93` `CONFIG_DIRS` → `[".kilo/"]` only (no `".kilocode/"`) and `isAbsolute()` → `~/.kilo` only via `KilocodePaths.globalDirs()`. Preserves `theme.tsx:564` `[".kilocode",".kilo"]` (TUI theme/plugin compatibility, P4.5-adjacent) per LOCK-004 and `kilocode/config/config.ts` `isConfigDir()` canonical `".kilo"` — **update 2026-08-28 bounded P4.4: `detectOpencodeConfig()` reference-only notification (not effective config) deleted (no `.opencode` scanner remains; cloud notifications remain; does not close P4.4/P4.5/P5)**. Tests: `paths.test.ts`/`project-id.test.ts`/`permission/config-paths.test.ts` legacy rewritten to absence, `p4-4-legacy-filesystem-discovery-removal.test.ts` (static absence + canonical preservation + runtime `false`/`true`, sorted profile `global-health < legacy < managed`). Docs: matrix rows 5,7,12,13 remain residual/open per LOCK-001 (no broader completion); runtime §8.1 historical sources already canonical-only. `KILO_CONFIG_DIR -> Global.Path.config` override and sandbox deny preserved per LOCK-005; no provider/transport/storage/convergence/SDK change.
...
| Preset provider catalog/onboarding | `packages/opencode/src/kilocode/provider/provider.ts` (deleted P4.4-T6 — `KILO_BUNDLED_PROVIDERS` ` "@kilocode/kilo-gateway": async () => createKilo` and `createKilo`/`BundledSDK` removed; `KILO_MODEL_SCHEMA_EXTENSIONS`/`patchModelsDevModel`/`kiloCustomLoaders` retained); `packages/opencode/src/kilocode/provider/models-api.json` (deleted P4.4-G2 2026-09-01 — 3.0 MB catalog removed, no preset snapshot remains); `packages/opencode/src/kilocode/provider/metadata.ts` (deleted P4.4-T5 — preset `providerMetadata` `noteKey`/`icon`/`priority` helper removed; `packages/opencode/src/server/routes/instance/httpapi/handlers/provider.ts` `metadata: providerMetadata(item.id)` enrichment removed, optional `Provider.Info` `metadata` field and `Provider` OpenAPI/SDK `metadata` remain optional/contract-compatible per LOCK-009); `packages/opencode/src/provider/provider.ts:178` (`BUNDLED_PROVIDERS` generic SDK loaders retained; `...KILO_BUNDLED_PROVIDERS` spread deleted P4.4-T6); `packages/opencode/src/provider/provider.ts:1383-1392` (deleted P4.4-G2 2026-09-01 — models.dev service deleted; no runtime fetch/fallback remains, explicit-config-only); `packages/opencode/src/provider/models.ts` (deleted P4.4-G2 2026-09-01 — file deleted; P4.4-T7 `kilo`/`apertis` injection and `ModelCache`/`KILO_OPENROUTER_BASE` removal is historical and superseded, no static Core overlay remains, explicit-config-only); `packages/opencode/src/provider/model-cache.ts` (deleted 2026-08-27 P4.4 residual package — file physically deleted, no ModelCache service remains; P4.4-T7 invalidation-only `clear` is historical and superseded) | Preset provider identities/catalogs, models.dev dependency (partial removal: preset display metadata enrichment removed P4.4-T5; preset `@kilocode/kilo-gateway` bundled loader removed P4.4-T6; kilo/apertis dynamic fetch/cache removed P4.4-T7 — `provider/model-cache.ts` `kilo`/`apertis` network deleted (file deleted P4.4-G2 superseded), `provider/models.ts` file deleted P4.4-G2 (P4.4-T7 static overlay is historical and superseded), handler `ModelCache` removed (file deleted); committed `models-api.json` deleted P4.4-G2; `Core.ModelsDev` (`packages/core/src/models-dev.ts`) deleted P4.4-G2 (disk cache/network/refresh removed), generic adapters, `KILO_MODEL_SCHEMA_EXTENSIONS`/`patchModelsDevModel`, custom-provider paths, HTTP/SSE bridge, and optional `metadata` schema remain open per LOCK-006/009 — `ModelCache` service deleted 2026-08-27 (no `clear` remains) and only `kilo`/`apertis` dynamic fetching was removed) | Removed (LOCK-006) — preset display metadata enrichment removed (P4.4-T5); preset `@kilocode/kilo-gateway` bundled loader removed (P4.4-T6); kilo/apertis dynamic fetch/cache removed (P4.4-T7; file deleted P4.4-G2 superseded) — `provider/model-cache.ts` file deleted 2026-08-27 (P4.4-G2 superseded), `provider/models.ts` file deleted P4.4-G2 (historical static overlay superseded), handler `ModelCache` file deleted; catalog (`models-api.json` deleted P4.4-G2; `packages/core/src/models-dev.ts` deleted P4.4-G2) — generic adapters/bridge remain, preset catalog fully removed, explicit-config-only (LOCK-006) |
...
| Checked-in catalog (models-dev snapshot) | `kilocode/provider/models-api.json` (deleted P4.4-G2 — file removed, 3.0 MB snapshot deleted) |
| Preset metadata keys | `kilocode/provider/metadata.ts` (deleted P4.4-T5 — preset `providerMetadata` helper removed; optional `Provider.Info` `metadata` field remains optional per LOCK-009; bundled loader deleted P4.4-T6 — `models-api.json` deleted P4.4-G2, `models.dev` runtime deleted P4.4-G2, generic adapters remain per LOCK-006/009; `ModelCache` deleted 2026-08-27) |
| Models.dev runtime fetch + empty-catalog fallback | Deleted P4.4-G2 2026-09-01 — `provider/provider.ts:1383-1392` models.dev service and `provider/models.ts:47-51` ("models.dev catalog unavailable, using empty catalog") fallback deleted; no runtime fetch or empty-catalog fallback remains |
...
| R-5 | 3.0 MB checked-in models catalog + preset loaders | `kilocode/provider/models-api.json` (deleted P4.4-G2 2026-09-01 — 3.0 MB catalog removed); `kilocode/provider/provider.ts` (deleted P4.4-T6 — `KILO_BUNDLED_PROVIDERS` preset loader deleted/historical; `KILO_MODEL_SCHEMA_EXTENSIONS`/`patchModelsDevModel`/`kiloCustomLoaders` retained as active/open); `provider/provider.ts:178` (`BUNDLED_PROVIDERS` generic SDK loaders retained; models.dev service deleted P4.4-G2) and `provider/models.ts` (deleted P4.4-G2 2026-09-01 — file deleted; P4.4-T7 `kilo`/`apertis` injection and `ModelCache` removal is historical and superseded, no static Core wrapper remains, explicit-config-only) and `provider/model-cache.ts` (deleted 2026-08-27 P4.4 residual package — file physically deleted, no ModelCache service remains; P4.4-T7 invalidation-only `clear` is historical and superseded); `kilocode/provider/metadata.ts` (deleted P4.4-T5 — preset `providerMetadata` helper deleted/historical; optional `Provider.Info` `metadata` field remains optional per LOCK-009) | Config/provider init | LOCK-006 removal (P4.4) — preset display metadata enrichment removed (P4.4-T5); preset `@kilocode/kilo-gateway` bundled loader removed (P4.4-T6); kilo/apertis dynamic fetch/cache removed (P4.4-T7; file deleted P4.4-G2 superseded) — `provider/model-cache.ts` file deleted 2026-08-27 (P4.4-G2 superseded), `provider/models.ts` file deleted P4.4-G2 (historical static overlay superseded), handler `ModelCache` file deleted; committed `models-api.json` deleted P4.4-G2 and `Core.ModelsDev` (`packages/core/src/models-dev.ts`) deleted P4.4-G2 — generic adapters/bridge remain, preset catalog fully removed, explicit-config-only (LOCK-006) — only `kilo`/`apertis` dynamic fetching was removed and ModelCache boundary is deleted not replaced |
...
| H-9 | User-selected custom-provider models | `packages/opencode/src/provider/provider.ts` (custom providers), `kilocode/provider/provider.ts`; per-session model selector | Executed (H-9 fixture `:516-704`, custom provider model) | Selector independence unproven; preset-provider catalog/models.dev removed P4.4-G2 (explicit-config-only, LOCK-006) |
