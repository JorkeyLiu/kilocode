...
> **Status: R18 EVALUATOR MATRIX COMPLETE — No phase/row closure claimed. P4.4 remains Active/residual; P4 remains Active.**
...
| Status | R18 evaluator matrix and production-path evidence complete — P4.4 Active/residual, P4 Active; no transport/phase row closed |
...
| Full evaluator matrix through production path | The added matrix tests cover hard deny, protected/env ceilings, exact approval, allow-everything ordinary-only behavior, no-rule/default behavior, decisive rule provenance, child deny/approval boundaries, and separate `question`/`question_tool` identifiers |

## Production-Path Test Matrix (new suite)

| Test | File:line | What it proves |
|---|---|---|
| hard deny remains absolute and provenance identifies the decisive runtime rule | `packages/opencode/test/permission/r18-production-path.test.ts:69` | Hard deny via `hardRuleset rm *` dominates `global: bash *: allow` → `deny` with `ceiling-a`, provenance `runtime-safety` `deny` `rm *` |
| exact approval resolves an ordinary ask and a protected ceiling in one operation | `packages/opencode/test/permission/r18-production-path.test.ts:99` | Single exact `session` approval for `kilo.json` resolves both ordinary `ask` and `ask-ceiling (b)` in one operation |
| allow-everything resolves ordinary ask but never protected ask-ceiling | `packages/opencode/test/permission/r18-production-path.test.ts:138` | `allowEverything` resolves ordinary `bash printf ordinary` to `allow` but `edit kilo.json` remains `ask-ceiling (b)` |
| child inherited deny reaches Permission.ask while parent approval stays session-scoped | `packages/opencode/test/permission/r18-production-path.test.ts:186` | `deriveSubagentSessionPermission` inherited `edit *: deny` reaches `Permission.ask` as `session-restriction-deny`; parent `session` approval for `kilo.json` not inherited by child |
| same-document exact rule wins and provenance identifies that rule | `packages/opencode/test/permission/r18-production-path.test.ts:253` | Within single `project` document `bash *: allow` + `git status: deny`, later exact `git status` wins → `deny` with decisive `project-deny` |
| question and question_tool remain separate through Permission.ask | `packages/opencode/test/permission/r18-production-path.test.ts:281` | `global: question deny` vs `question_tool allow` remain separate identities via `Permission.ask` and provenance |
| absent global with project allow gives immediate allow | `packages/opencode/test/permission/r18-production-path.test.ts:326` | Absent global is non-applicable; project `bash *: allow` yields immediate `allow` with `project-file` provenance, no `global-file` |
| authored empty global is applicable ask versus absent allows | `packages/opencode/test/permission/r18-production-path.test.ts:354` | Empty `{"permission":{}}` global file is applicable `ask` that blocks project allow; provenance `global-file` `ask`; absent allows |
| global deny via real disk dominates project allow | `packages/opencode/test/permission/r18-production-path.test.ts:393` | Disk `global: bash *: deny` dominates `project: bash *: allow` → `deny` with `global-file` `deny` |
| project absent with global allow still allows | `packages/opencode/test/permission/r18-production-path.test.ts:430` | Single global allow with no project file yields `allow` |
| ceiling b ask-ceiling and exact approval positive/negative | `packages/opencode/test/permission/r18-production-path.test.ts:454` | `edit kilo.json` with `project: edit *: allow` caps at `ask-ceiling (b)`; exact `session` resolves; wildcard selector `*` never stored as wildcard identity (for exact pending request, `*` normalizes to exact canonical if retained), cross-session not inherited |
| ceiling c broad allow caps and exact approval resolves | `packages/opencode/test/permission/r18-production-path.test.ts:530` | `read secret.env` with `project: read *: allow` caps at `ask-ceiling (c)`; exact `session` for `secret.env` resolves, `other.env` not, `*.env.example` not capped |
| saveAlwaysRules c exact vs wildcard/broad | `packages/opencode/test/permission/r18-production-path.test.ts:602` | `saveAlwaysRules` exact selector `secret.env` resolves only that target; `other.env` remains `ask-ceiling`; wildcard selector `*.env` never stored as wildcard identity and remains `ask-ceiling`; broad selector `*` never stored as `*` — for exact pending request it may normalize to exact canonical if retained |
| once consumed after one op and no file mutation | `packages/opencode/test/permission/r18-production-path.test.ts:728` | `once` approval for `bash` allows one `operationId` then next same pattern pending; `kilo.jsonc` file unchanged |
| session persists until dispose and no durable mutation | `packages/opencode/test/permission/r18-production-path.test.ts:772` | `session` approval for `edit kilo.json` persists in same session, `InstanceState.dispose` drops it, file unchanged |
| child does not inherit parent session approval | `packages/opencode/test/permission/r18-production-path.test.ts:837` | Parent `sess_parent_approval` session approval for `kilo.json` not visible to `sess_child_approval` |
| child deny isolation | `packages/opencode/test/permission/r18-production-path.test.ts:892` | Session deny via `__testSetSessionRules` for `sess_parent_deny` does not leak to `sess_child_deny` |
| provenance correctness through service | `packages/opencode/test/permission/r18-production-path.test.ts:931` | `deny`/`ask`/`allow` provenance via service has `runtime-safety` first, correct `sourceKind`/`canonicalPath`/`decision`, `schemaVersion 1`, `request` ids, `approval` metadata |
...
| Command | Result |
|---|---|
| `bun test ./test/permission/r18-production-path.test.ts` (bounded subgate, 2026-08-28) | PASS — 12 pass 77 expect |
| `bun test --isolate ./test/permission/r18-evaluator.test.ts` | PASS — 64 pass 166 expect |
| `bun test ./test/permission/r18-production-path.test.ts` | PASS — 18 pass 106 expect |
| `bun test ./test/permission/r18-production-path.test.ts --test-name-pattern 'full evaluator matrix'` | PASS — 6 pass 29 expect |
...
- P4.4/P4.5/P5 completion: not claimed. The evaluator matrix closes R18's bounded implementation/test evidence only; transport bridges and unrelated phase gates remain open.
...
| Date | Change |
|---|---|
| 2026-08-29 | Full evaluator matrix closure evidence added: six production-path tests prove hard deny and decisive-rule provenance, exact mixed ask/ceiling approval, ordinary-only allow-everything, child inherited deny plus non-inherited parent approval, same-document decisive ordering, and separate `question`/`question_tool` identities. The canonical config schema now accepts `question_tool` as a scalar permission ID. Direct evaluator and complete production-path runs pass; P4.4/P4.5/P5 remain outside scope. |
