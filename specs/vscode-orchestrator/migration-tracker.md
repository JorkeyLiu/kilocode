...
removal; P3.2-P3.4, P4, and P5 remain Not started; R2 and R6 were revised by a
...
Current phase: **P3 Active — P3.1 Complete (2026-08-15)**; **P3.2-P3.4, P4, P5 Not started** (P0 Complete 2026-08-12; P1 Complete 2026-08-14; P2 exit evidence recorded in sections 4, 5, 6, 8; P3.1 exit evidence recorded in sections 4, 5, 7, 8; P3 overall is Active, never Complete until P3.4 exits).
...
| P3 | Product removal | Active — P3.1 Complete (2026-08-15); P3.2-P3.4 Not started | test: `packages/kilo-vscode/tests/unit/sidebar-removal.test.ts`, `packages/kilo-vscode/tests/unit/chat-target.test.ts`, `packages/kilo-vscode/tests/e2e/runner.ts`, `packages/kilo-vscode/script/e2e-probe.ts` (focused real Extension Host `test:e2e:sidebar-removal` passed); doc: `packages/kilo-docs/pages/contributing/architecture/vscode-extension.md`, `specs/vscode-orchestrator/evidence/p0-baseline/p3-1-sidebar-2026-08-15T11-19-02-000Z/benchmark.jsonl`, `.changeset/remove-sidebar-chat.md` | P3.1 (sidebar, LOCK-001) Complete 2026-08-15 with per-category removal evidence (section 7), rollback = git revert of the P3.1 removal commit(s) (no feature flag/shim), and a descriptive scenario-1 cold-start performance comparison vs the P0 baseline (section 8); P3.2-P3.4 (worktree/diff, cloud/JetBrains/Console/KiloClaw, indexing/memory/context-management/autocomplete) Not started; P3 overall is Active, never Complete until P3.4 exits |
...
- Status: Active — P3.1 Complete (2026-08-15); P3.2-P3.4 Not started. P3 overall
...
- Next actions: P3.2 (worktree infrastructure + custom Diff Viewer surfaces, LOCK-002) is the next subphase and remains Not started; P3.3/P3.4 follow. Keep the P3.1 static contract and chat-target unit tests and the `test:e2e:sidebar-removal` scenario green.
...
| H-12 | Checkpoint rollback | From a panel-hosted session, withdrawing/reverting a message restores the affected code state, the revert can be un-reverted or cleaned up, and lifecycle stays correct; distinct from the ADR-0005 canonical storage foundation and ADR-0001's historical checkpoint/resync | Proven at P2 (2026-08-15, migration bridge) | - | test: `packages/opencode/test/kilocode/p0-harness-baseline.test.ts`; doc: `specs/vscode-orchestrator/evidence/p0-baseline/2026-08-15T08-35-00Z-p2/real-completed/` — `real-completed-dom-evidence` `rollback` (file `rollback.txt`, boundaryMessageID, editedBytes "E2E_ROLLBACK_EDITED\n"), `workspace/rollback.txt`, run.log "PASS H-12 exact initial bytes restored" (`E2E_ROLLBACK_ORIGINAL\n`), "PASS H-12 RevertBanner visible" + lists the file, "PASS H-12 edited bytes restored by Redo All", "PASS H-12 RevertBanner gone after Redo All", reverted turns re-shown |
...
**Complete (2026-08-15)** with per-category objective evidence, and the remaining
rows stay residue-only. `Unknown` means the category may exist
...
|---|---|---|---|---|---|
| Webview message types | 332 distinct `type` literals (Q3 approved 2026-08-12; inventory §4.4/§9) | 0 (P3.1, 2026-08-15: recount 332, delta 0 — the removed sidebar shared the editor-tab/Agent Manager webview bundle, so no distinct message-type retirement changes the total) | TBD | TBD | doc: `specs/vscode-orchestrator/p0-current-state-inventory.md` (§4.4 reproducible count) |
| Provider methods | 250 v2 SDK public methods (Q3 approved 2026-08-12; inventory §4.3/§9) | 0 (P3.1, 2026-08-15: recount 250, delta 0 — no SDK contract change; generated SDK `-` by nature for this removal) | TBD | TBD | doc: `specs/vscode-orchestrator/p0-current-state-inventory.md` (§4.4 reproducible count) |
| Webview entry points | 6 esbuild webview entry points (Q3 approved 2026-08-12; inventory §4.4/§9) | 0 (P3.1, 2026-08-15: recount 6, delta 0 — the sidebar shared the `webview-ui/src/index.tsx` editor-chat entry; no entry retired) | TBD | TBD | doc: `specs/vscode-orchestrator/p0-current-state-inventory.md` (§4.4 reproducible count) |
...
| Removal items with full evidence | 0 (no removal complete; residue only, inventory §5) | 1 (P3.1, 2026-08-15: ordinary single-chat sidebar row in section 7 complete; P3.2-P3.4 deltas pending) | TBD | TBD | doc: `specs/vscode-orchestrator/p0-current-state-inventory.md` (§5); tracker §7 |

The `Delta at P3` column above reflects **P3.1 only** (2026-08-15): the sidebar
removal shared the editor-tab/Agent Manager webview bundle and made no
generated-SDK contract change, so message types, provider methods, and webview
entry points all recount at their P0 baseline (332/250/6, each delta 0) — a
zero-delta boundary explanation, not incomplete removal. P3.2-P3.4 complexity
and removal deltas remain pending and stay `TBD` until their subphases record
evidence.
...
| No-provider startup (fresh, no seeded config; startup without provider/auth records) | Measured (n=5, descriptive p95=max; scope: cold-start campaign condition = fresh VS Code profile + fresh scratch XDG, no seeded config, providers 0, git 461781ffc0): `spawnToPortMs` med 5382 / p95 6652 ms; `activateToDataReadyMs` med 9231 / p95 10189 ms — not an SLA or threshold (R7 resolved 2026-08-14) | P3.1 descriptive (2026-08-15, scenario 1 cold-start, same fresh no-seed condition, git 67436bdf70 dirty, VS Code 1.133.0): `spawnToPortMs` med 9407 / p95=max 13684 ms; `activateToDataReadyMs` med 16772 / p95=max 21748 ms — descriptive n=5 only, no threshold, no improvement/regression claim (R7) | TBD | TBD | doc: `specs/vscode-orchestrator/evidence/p0-baseline/2026-08-11T11-51-16-883Z/benchmark.jsonl`; P3.1: doc: `specs/vscode-orchestrator/evidence/p0-baseline/p3-1-sidebar-2026-08-15T11-19-02-000Z/benchmark.jsonl` |
...
- Canonical architecture docs: `packages/kilo-docs/pages/contributing/architecture/` (LOCK-013; `vscode-extension.md` updated in the P2 work unit to document the implemented env-gated fixture bridge and in the P3.1 work unit to document the implemented sidebar removal)
