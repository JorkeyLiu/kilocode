...
| P4.4-G3-B2 | P4.4 | B2 durable `session/update` title-only lane + private `session/update` fd3/fd4 parity into same AppLayer (SDK `PATCH` authoritative, private `dispatchPrivate` replay-only, per-operation `sessionUpdate:<id>:<token>` distinct, `idempotencyKey` shared, one `title`+`revision`+`changefeed`+`result_snapshot` commit, same-key replay returns serialized `SessionTable` row converted/validated as `Session.Info`/`revision` without feed, HTTP `context.sessionId` binding vs URL, private strict validation parity, null `parentSessionId` only, **private `opId` collision, reader-failure replay, barrier with single continuous `GenerationGate` lease (statically verified plus active-barrier test), malformed object snapshot, concurrency, rollback**) — `SessionUpdateDispatch` + `SessionOperation` `result_snapshot` + `fd-carrier` `session/update` via `dispatchPrivate` + `KiloProvider` shared-token `buildSessionUpdateIdentity` + `compareUpdateParity` | Active | [doc: evidence/p4.4-g3-b2-private-session-update.md](evidence/p4.4-g3-b2-private-session-update.md); [test: packages/opencode/test/kilocode/session/session-update-b2.test.ts](../../packages/opencode/test/kilocode/session/session-update-b2.test.ts) (27); [test: packages/core/test/session-update-operation.test.ts](../../packages/core/test/session-update-operation.test.ts); [test: packages/kilo-vscode/src/services/cli-backend/serve-private-peer-b2.test.ts](../../packages/kilo-vscode/src/services/cli-backend/serve-private-peer-b2.test.ts); [test: packages/kilo-vscode/src/kilo-provider-rename-b2.test.ts](../../packages/kilo-vscode/src/kilo-provider-rename-b2.test.ts); [doc: packages/kilo-docs/pages/contributing/architecture/cli-runtime.md#durable-title-only--sessionupdate-g3-b2](../../packages/kilo-docs/pages/contributing/architecture/cli-runtime.md#durable-title-only--sessionupdate-g3-b2) — G3 remains Active, SDK authoritative, no cutover, no Gates C-F/P4.4 closure, no Linux/Windows production proof |
