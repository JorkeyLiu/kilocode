...
legacy storage and transport sources removed with implementation),
...
Failure/Outcome/Recovery is a bounded runtime-owned target described in
`../vscode-orchestrator/direction.md`, not a separate technical spec and not an
exhaustive new ADR. It establishes a runtime-owned
...
- I-2 (LOCK-014): the two closed sets are fixed. Final required capabilities
  ...
  implementation, and legacy storage and transport sources removed with
...
- I-9: Failure/Outcome/Recovery is a bounded runtime-owned target recorded in `../vscode-orchestrator/direction.md`. It establishes a
...
- I-10: four bounded implementation decisions — R11 operation/outcome identity,
  ...
  bounded implementation decisions owned by the implementing work. No 14-variant
...
- I-12: post-reconstruction error maturity — taxonomy refinement, diagnostic
  ...
  planned after core reconstruction and stays out of the direction target. It is not a reconstruction
  candidate, never creates just-in-time registry entries, and does not block
  P4/P5 unless separately promoted.

The detailed just-in-time rules live with the implementing work; the
Failure/Outcome/Recovery target lives in `../vscode-orchestrator/direction.md`.
...
| A separate Failure/Outcome/Recovery technical spec or exhaustive ADR | Rejected for now: `../vscode-orchestrator/direction.md` owns the bounded target without a separate spec |
...
| Track error maturity items as just-in-time reconstruction candidates | Rejected: maturity items are forward-looking work, not displaced legacy behaviors; they never enter the just-in-time registry (I-12) |
...
- Related decisions: ADR-0002 (product direction and removals) and ADR-0003
  ...
  ADR-0005 (`../adr/0005-bounded-private-runtime-storage.md`)
